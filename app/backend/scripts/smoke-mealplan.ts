...
function pool(): MealCandidate[] {
  return [
    { id: 'chicken', name: 'Pollo', kcal100: 165, prot100: 31, carb100: 0, fat100: 3.6, priorityRank: 0, userOwned: true },
    { id: 'rice', name: 'Arroz', kcal100: 130, prot100: 2.7, carb100: 28, fat100: 0.3, priorityRank: 1, userOwned: true },
    { id: 'egg', name: 'Huevo', kcal100: 143, prot100: 13, carb100: 1.1, fat100: 9.5, priorityRank: 2, userOwned: true },
    { id: 'broccoli', name: 'Brocoli', kcal100: 34, prot100: 2.8, carb100: 7, fat100: 0.4, priorityRank: 2, userOwned: true },
    { id: 'oats', name: 'Avena', kcal100: 389, prot100: 17, carb100: 66, fat100: 7, priorityRank: 4, userOwned: false },
    { id: 'sweetpotato', name: 'Camote', kcal100: 86, prot100: 1.6, carb100: 20, fat100: 0.1, priorityRank: 4, userOwned: false },
  ];
}
function mkDecision(code: PlanDecisionCode, over: Partial<PlanDecision> = {}): PlanDecision {
  return { code, dimension: 'CALORIES', confidence: 'MEDIUM', explanation: '', evidence: [], adjustment: null, reviewWindowDays: 7, ...over };
}
function mkPlan(headline: PlanDecision, decisions: PlanDecision[], posture: any = 'STABLE'): NutritionPlan {
  return { meta: { planner: 'vitals-fit.adaptive-planner', version: 1, contractVersion: 1, weeksAnalyzed: 3, generatedAt: '2026-07-14T00:00:00.000Z' }, posture, headline, decisions, reviewWindowDays: headline.reviewWindowDays };
}
...
async function main() {
  ...
  check('3 meals for a standard lose plan', base.meals.length === 3 && base.meals.map((m) => m.slot).join(',') === 'BREAKFAST,LUNCH,DINNER');
  check('targets source = current-goal (no planner change)', base.targets.source === 'current-goal' && base.targets.calories === 2000);
  check('every meal has at least a protein anchor', base.meals.every((m) => m.items.length >= 1));
  check('totals land near the calorie target', Math.abs(base.totals.calories - 2000) / 2000 <= 0.25, `${base.totals.calories}`);
  check('coverage counts user-owned foods', base.coverage.fromUserFoods > 0 && base.coverage.totalItems >= base.coverage.fromUserFoods);
  check('protein anchor is the favorite (Pollo)', base.meals[0].items[0].name === 'Pollo' && base.meals[0].items[0].source === 'favorite');
  ...
  const cut = mkDecision('REDUCE_CALORIES', { adjustment: { calorieDelta: -200, newCalorieTarget: 1800 }, reviewWindowDays: 14 });
  const reducePlan = buildMealPlan(mkInput({ plan: mkPlan(cut, [cut], 'EVOLVING') }));
  check('meal plan executes the planner-adjusted calories (1800)', reducePlan.targets.source === 'planner-adjusted' && reducePlan.targets.calories === 1800, `${reducePlan.targets.calories}`);
  ...
  check('breakfast gets more protein than dinner', proteinLow.meals[0].targetProteinG > proteinLow.meals[2].targetProteinG, `${proteinLow.meals[0].targetProteinG} vs ${proteinLow.meals[2].targetProteinG}`);

  const adherenceFirst = buildMealPlan(mkInput({ plan: mkPlan(mkDecision('KEEP_PLAN'), [mkDecision('KEEP_PLAN')], 'ADHERENCE_FIRST') }));
  check('ADHERENCE_FIRST -> SIMPLER_STRUCTURE', adherenceFirst.adaptations.includes('SIMPLER_STRUCTURE'));
  check('simpler = at most 2 items per meal', adherenceFirst.meals.every((m) => m.items.length <= 2));

  const gain = buildMealPlan(mkInput({ goalDirection: 'gain', goalTargets: { calories: 2900, proteinG: 160, carbsG: 320, fatG: 80 } }));
  check('gain (not simpler) -> EXTRA_SNACK, 4 meals', gain.adaptations.includes('EXTRA_SNACK') && gain.meals.length === 4);

  console.log('\n── DETERMINISM & HYGIENE ──');
  check('identical input -> identical plan', JSON.stringify(strip(buildMealPlan(mkInput()))) === JSON.stringify(strip(buildMealPlan(mkInput()))));
  const json = JSON.stringify(base);
  check('no userId / raw-log keys leaked', !json.includes('userId') && !json.includes('dailyLog') && !json.includes('loggedMeal'));
  ...
  await prisma.userProfile.create({ data: { userId: user.id, name: 'M', age: 30, weightKg: 80, heightCm: 178, sex: 'MALE', activityLevel: 'MODERATE', fitnessLevel: 'BEGINNER' } });
  ...
  const mkFood = (name: string, kcal: number, p: number, c: number, f: number) =>
    prisma.foodItem.create({ data: { name, nameLower: name.toLowerCase(), nameNormalized: name.toLowerCase(), nameAliases: [], caloriesPer100g: kcal, proteinPer100g: p, carbsPer100g: c, fatPer100g: f, fiberPer100g: 0, source: 'curated_latam', isCommon: true } });
  ...
  check('targets from current goal (no completed weeks -> planner WAITs)', mp.targets.source === 'current-goal' && mp.targets.calories === 2000, `${mp.targets.calories}`);
  check('plan uses the favorited food (Pollo, source favorite)', mp.meals.some((m) => m.items.some((it) => it.name === 'Pollo' && it.source === 'favorite')));
  check('coverage counts the user food', mp.coverage.fromUserFoods > 0);
  check('service plan is deterministic on rebuild', JSON.stringify(strip(await mealPlanner.getMealPlan(user.id))) === JSON.stringify(strip(mp)));
