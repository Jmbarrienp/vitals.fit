...
function gov(over: Partial<GovernanceRecommendation> = {}): GovernanceRecommendation {
  return {
    ...
    evidence: {
      ...
      top1DeltaCi: null,
      mcNemarZ: null,
      generalizesAcrossModalities: null,
      generalizesAcrossUsers: null,
      latencyDeltaMs: null,
      tokenDeltaPerScan: null,
      challengerAvailability: null,
...
const gates = (
  rollbackStatus: 'PASS' | 'FAIL' = 'PASS',
  rollbackReasons: string[] = ['salud en verde'],
): GatesReport => ({
  contractVersion: 1,
  window: W,
  gates: [
...
const health = (over: Partial<HealthReport> = {}): HealthReport => ({
  ...
  promotion: { status: 'NOT_EVALUATED', detail: '' },
  falsePositives: 0,
  falseNegatives: 1,
  ...over,
});
const rollout = (autoAcceptEnabled: boolean, provider = 'alpha'): RolloutStatus => ({
  contractVersion: 1,
  window: W,
  generatedFor: { activeProviderId: provider, autoAcceptEnabled },
...
const promo = (readiness: 'READY' | 'BLOCKED' | 'NOT_APPLICABLE' = 'BLOCKED'): PromotionExecutionPlan => ({
  ...
  estimatedRisk: { overall: 'LOW', dimensions: [] },
  rolloutStrategy: [],
  rolloutPercent: 0,
...
async function main() {
  ...
  check(
    'healthy state -> NOT_REQUIRED, target NONE, severity NONE',
    calm.readiness === 'NOT_REQUIRED' && calm.rollbackTarget.kind === 'NONE' && calm.rollbackSeverity === 'NONE',
  );
  check(
    '…priority NONE and a no-trigger confidence',
    calm.rollbackPriority === 'NONE' && calm.rollbackConfidence === 'SIN_DISPARADOR',
  );
  check(
    '…no rollback steps for a calm state',
    calm.rollbackSteps.length === 0 && calm.rollbackReason.includes('no se requiere'),
  );

  console.log('\n── V4.3: CAUSE — degraded health / undo (the canonical gate trigger) ──');
  const undoTrigger = buildRollbackPlan(
    gov(),
    risk('LOW'),
    gates('FAIL', ['REVERTIR: tasa de undo 50% > 20%']),
    health({ undoRate: 0.5 }),
    rollout(true),
    promo(),
    AT,
  ...
  check(
    'ROLLBACK_REQUIRED FAIL + auto-accept on -> REQUIRED, DISABLE_AUTO_ACCEPT',
    undoTrigger.readiness === 'REQUIRED' && undoTrigger.rollbackTarget.kind === 'DISABLE_AUTO_ACCEPT',
  );
  check(
    '…the trigger evidence is the gate reasons VERBATIM (not re-derived)',
    undoTrigger.triggeringEvidence.some((e) => e.includes('tasa de undo 50%')),
  );
  check(
    '…severity HIGH (gate fired, no false positives yet)',
    undoTrigger.rollbackSeverity === 'HIGH' && undoTrigger.rollbackPriority === 'SCHEDULED',
  );
  check(
    '…deterministic disable-auto-accept steps, containment first',
    undoTrigger.rollbackSteps[0].detail.includes('AUTO_ACCEPT_ENABLED=false'),
  );

  console.log('\n── V4.3: CAUSE — false positives (the platform acted wrong on users) ──');
  const fpTrigger = buildRollbackPlan(
    gov(),
    risk('LOW'),
    gates('FAIL', ['REVERTIR: 5 falsos positivos > 10']),
    health({ falsePositives: 5 }),
    rollout(true),
    promo(),
    AT,
  );
  check(
    'gate fired + false positives -> CRITICAL, IMMEDIATE',
    fpTrigger.rollbackSeverity === 'CRITICAL' && fpTrigger.rollbackPriority === 'IMMEDIATE',
  );
  check(
    '…the verification checklist flags the false positives as FAIL',
    fpTrigger.verificationChecklist.find((i) => i.label.includes('falsos positivos'))?.status === 'FAIL',
  );
  check(
    '…the communication plan escalates immediately',
    fpTrigger.communicationPlan.some((c) => c.includes('INMEDIATAMENTE')),
  );

  console.log('\n── V4.3: CAUSE — provider drift with auto-accept already off -> BLOCKED ──');
  const driftBlocked = buildRollbackPlan(
    gov({
      action: 'DEMOTE',
      evidence: { ...gov().evidence, incumbentDrift: 'DRIFTING' },
      reasons: ['top-1 cayó 10pp'],
    }),
    risk('MEDIUM'),
    gates('PASS'),
    health(),
  ...
  check(
    'drift + auto-accept off -> RESTORE_PROVIDER, but BLOCKED on operator',
    driftBlocked.readiness === 'BLOCKED' && driftBlocked.rollbackTarget.kind === 'RESTORE_PROVIDER',
  );
  check(
    '…because promotion history is not persisted (honest gap, named)',
    driftBlocked.blockingReasons.some((r) => r.includes('historial de promociones')),
  );
  check(
    '…the safe floor (fixture) is named as the target provider',
    driftBlocked.rollbackTarget.provider === 'fixture',
  );
  check(
    '…severity MEDIUM (drift, no acute health failure), priority MONITOR',
    driftBlocked.rollbackSeverity === 'MEDIUM' && driftBlocked.rollbackPriority === 'MONITOR',
  );
  check(
    '…restore-provider steps require operator confirmation first',
    driftBlocked.rollbackSteps[0].detail.includes('confirma el último proveedor bueno'),
  );

  console.log('\n── V4.3: CAUSE — HIGH risk escalates severity ──');
  const highRisk = buildRollbackPlan(
    gov(),
    risk('HIGH', [{ dimension: 'USER', level: 'HIGH', evidence: ['undo 50% — los usuarios revierten'] }]),
    gates('PASS'),
    health(),
    rollout(true),
    promo(),
  ...
  check(
    'HIGH risk alone triggers a rollback (auto-accept lever), CRITICAL',
    highRisk.readiness === 'REQUIRED' && highRisk.rollbackSeverity === 'CRITICAL',
  );
  check(
    '…the risk summary is quoted from the Risk Engine, not re-assessed',
    highRisk.riskSummary.overall === 'HIGH' && highRisk.riskSummary.dimensions[0].dimension === 'USER',
  );

  console.log('\n── V4.3: CONFIDENCE (a count of AGREEING signals, not a recomputation) ──');
  const oneSignal = buildRollbackPlan(
    gov(),
    risk('LOW'),
    gates('FAIL', ['REVERTIR: undo alto']),
    health({ undoRate: 0.5 }),
    rollout(true),
    promo(),
    AT,
  ...
  const threeSignals = buildRollbackPlan(
    gov({ action: 'DEMOTE', evidence: { ...gov().evidence, incumbentDrift: 'DRIFTING' } }),
    risk('HIGH'),
    gates('FAIL', ['REVERTIR: undo alto']),
    health({ undoRate: 0.5 }),
    rollout(true),
  ...
  check(
    'triggering metrics quote health with the OWNING thresholds',
    undoTrigger.triggeringMetrics.find((m) => m.metric === 'undoRate')?.observed === 0.5 &&
      undoTrigger.triggeringMetrics.find((m) => m.metric === 'undoRate')?.threshold === 0.2,
  );
  check(
    'each metric names its source owner',
    undoTrigger.triggeringMetrics.every((m) => !!m.source),
  );
  check(
    "degraded health is isHealthGreen()'s reasons verbatim",
    undoTrigger.degradedHealth.length > 0 && undoTrigger.degradedHealth[0].includes('undo'),
  );
  check(
    'failed gates are listed from the Gates report',
    undoTrigger.failedGates.some((g) => g.id === 'ROLLBACK_REQUIRED'),
  );
  check(
    'retry conditions reuse the Promotion Executor readiness, not new criteria',
    undoTrigger.retryConditions.some((r) => r.includes('V4.2')),
  );
  ...
  check(
    'every item fully typed — no bare booleans',
    allItems.every((i) => !!i.status && !!i.explanation && !!i.severity && !!i.owner && !!i.category),
  );
  check(
    'the six categories are represented across the plan',
    new Set(allItems.map((i) => i.category)).size >= 5,
  ...
  check(
    ...
    ) === JSON.stringify(oneSignal),
  );
  check(
    'generatedAt is an INPUT, stamped and versioned',
    undoTrigger.generatedAt === AT && undoTrigger.version === ROLLBACK_PLAN_VERSION,
  );
  ...
  check(
    ...
    plan.version === ROLLBACK_PLAN_VERSION && plan.generatedAt === AT,
  );
  check(
    'empty, healthy DB -> NOT_REQUIRED (no phantom rollback)',
    plan.readiness === 'NOT_REQUIRED' && plan.rollbackTarget.kind === 'NONE',
  );
  check(
    'projections agree with the full plan',
    (await engine.readiness(30, AT)).readiness === plan.readiness &&
      (await engine.summary(30, AT)).currentProvider === plan.currentProvider,
  );
