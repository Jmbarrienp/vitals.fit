...
const LADDER: RolloutStage[] = [5, 10, 25, 50, 100].map((percent, i) => ({
  percent, suggestedDurationHours: [24, 24, 48, 48, 72][i],
  advanceConditions: [`avanzar-${percent}`], stopConditions: [`parar-${percent}`], rollbackConditions: [`revertir-${percent}`],
}));

function promo(over: Partial<PromotionExecutionPlan> = {}): PromotionExecutionPlan {
  return {
    version: 1, generatedAt: AT, currentProvider: 'alpha', candidateProvider: 'beta', decision: 'PROMOTE',
    confidence: 'MEDIA', readiness: 'READY', blockingReasons: [], statisticalEvidence: {} as any,
    estimatedRisk: { overall: 'LOW', dimensions: [{ dimension: 'TECHNICAL', level: 'LOW', topEvidence: 'ok' }] },
    rolloutStrategy: LADDER, rolloutPercent: 5, estimatedDurationHours: 216, rollbackCriteria: [], rollbackSteps: [],
    executionSteps: [], monitoringChecklist: [], validationChecklist: [], approvalChecklist: [], window: W, ...over,
  };
}
function rb(over: Partial<RollbackExecutionPlan> = {}): RollbackExecutionPlan {
  return {
    version: 1, generatedAt: AT, currentProvider: 'alpha',
    rollbackTarget: { kind: 'NONE', provider: null, detail: 'sin rollback' },
    readiness: 'NOT_REQUIRED', rollbackReason: 'sin degradación', rollbackSeverity: 'NONE', rollbackPriority: 'NONE',
    rollbackConfidence: 'SIN_DISPARADOR', blockingReasons: [], triggeringEvidence: ['salud verde'], triggeringMetrics: [],
    failedGates: [], degradedHealth: [], riskSummary: { overall: 'LOW', dimensions: [] }, rollbackSteps: [],
    verificationChecklist: [], postRollbackChecklist: [], monitoringPlan: [], communicationPlan: [], retryConditions: [],
    estimatedImpact: [], rollbackWindow: W, ...over,
  };
}
...
async function main() {
  ...
  check('healthy, promotable, low risk, mid-ladder -> ADVANCE', advance.recommendation === 'ADVANCE' && advance.readiness === 'READY_TO_ADVANCE');
  ...
  check('green but MEDIUM risk -> STAY (keep observing, don\'t advance)', stay.recommendation === 'STAY' && stay.reason.includes('MEDIO'));

  const hold = decideCanary(0, promo({ readiness: 'BLOCKED', blockingReasons: ['Governance recomienda HOLD'] }), rb());
  check('no viable promotion -> HOLD, quoting the promotion blocker', hold.recommendation === 'HOLD' && hold.reason.includes('HOLD'));
  check('no candidate -> HOLD too', decideCanary(0, promo({ readiness: 'NOT_APPLICABLE' }), rb()).recommendation === 'HOLD');

  const pause = decideCanary(10, promo(), rb({ readiness: 'REQUIRED', rollbackPriority: 'MONITOR', rollbackSeverity: 'MEDIUM', degradedHealth: ['undo 12%'] }));
  check('mild degradation (rollback at MONITOR) -> PAUSE (contain, don\'t revert)', pause.recommendation === 'PAUSE' && pause.reason.includes('degradación leve'));

  const rollbackImmediate = decideCanary(25, promo(), rb({ readiness: 'REQUIRED', rollbackPriority: 'IMMEDIATE', rollbackSeverity: 'CRITICAL', rollbackReason: 'undo 50%' }));
  check('IMMEDIATE rollback priority -> ROLLBACK (abort), deferring to the Rollback plan', rollbackImmediate.recommendation === 'ROLLBACK' && rollbackImmediate.readiness === 'ABORTING');
  check('SCHEDULED rollback priority also aborts', decideCanary(25, promo(), rb({ readiness: 'REQUIRED', rollbackPriority: 'SCHEDULED', rollbackSeverity: 'HIGH' })).recommendation === 'ROLLBACK');
  check('abort OUTRANKS everything: even with a promotable plan, ROLLBACK wins', decideCanary(50, promo({ readiness: 'READY' }), rb({ readiness: 'REQUIRED', rollbackPriority: 'IMMEDIATE' })).recommendation === 'ROLLBACK');

  const complete = decideCanary(100, promo(), rb());
  check('at 100%, promotable, no degradation -> COMPLETE', complete.recommendation === 'COMPLETE' && complete.readiness === 'COMPLETED');

  console.log('\n── V4.4: PURE CONSUMER (ladder + abort signal both consumed, not recomputed) ──');
  const plan = buildCanaryPlan(5, promo(), rb(), AT);
  check('the timeline IS the Promotion plan\'s ladder (5 rungs, same durations)', plan.timeline.map((s) => s.percent).join(',') === '5,10,25,50,100' && plan.timeline[4].suggestedDurationHours === 72);
  check('required conditions are the current rung\'s advanceConditions, verbatim', plan.requiredConditions.join(',') === 'avanzar-5');
  check('the promotion reference points, never copies', plan.promotionReference.readiness === 'READY' && plan.promotionReference.candidateProvider === 'beta');
  check('the rollback reference points, never copies', plan.rollbackReference.readiness === 'NOT_REQUIRED' && plan.rollbackReference.priority === 'NONE');
  ...
  check('pre-rollout (0%): no CURRENT rung, next is 5%, all rungs FUTURE', pre.currentStage === null && pre.nextStage?.percent === 5 && pre.timeline.every((s) => s.position === 'FUTURE'));
  const at10 = buildCanaryPlan(10, promo(), rb(), AT);
  check('at 10%: 5% is PAST, 10% is CURRENT, 25%+ are FUTURE', at10.timeline[0].position === 'PAST' && at10.timeline[1].position === 'CURRENT' && at10.timeline[2].position === 'FUTURE');
  check('…current stage carries live observed indicators', at10.currentStage?.observedIndicators.length! > 0 && at10.currentStage?.observedIndicators.some((i) => i.includes('ADVANCE')));
  check('…next stage is 25%', at10.nextStage?.percent === 25);
  const at7 = buildCanaryPlan(7, promo(), rb(), AT);
  check('an off-rung position (7%) snaps to the rung it is on (5% CURRENT, 10% next)', at7.currentStage?.percent === 5 && at7.nextStage?.percent === 10);
  const at100 = buildCanaryPlan(100, promo(), rb(), AT);
  check('at 100%: it is CURRENT, everything below is PAST, no next stage', at100.currentStage?.percent === 100 && at100.nextStage === null && at100.timeline[0].position === 'PAST');
  ...
  check('advance/hold/rollback signals each carry an explanation', plan.advanceRecommendation.explanation.length > 0 && plan.holdRecommendation.explanation.length > 0 && plan.rollbackRecommendation.explanation.length > 0);
  check('advance signal true only when the recommendation is ADVANCE', plan.advanceRecommendation.value === true && buildCanaryPlan(5, promo({ estimatedRisk: { overall: 'MEDIUM', dimensions: [] } }), rb(), AT).advanceRecommendation.value === false);
  check('exposure names the current and next percent', plan.estimatedExposure.currentPercent === 5 && plan.estimatedExposure.nextPercent === 10);
  check('at 100% there is no next exposure', at100.estimatedExposure.nextPercent === null);
  check('checklists are fully typed', [...plan.monitoringChecklist, ...plan.verificationChecklist].every((i) => !!i.status && !!i.explanation && !!i.owner && !!i.category));

  console.log('\n── V4.4: DETERMINISM + IDEMPOTENCY ──');
  check('same inputs + same timestamp -> byte-identical plan', JSON.stringify(buildCanaryPlan(10, promo(), rb(), AT)) === JSON.stringify(at10));
  ...
  check('the engine produces a versioned plan over an empty DB without crashing', realPlan.version === CANARY_PLAN_VERSION && realPlan.generatedAt === AT);
  check('empty DB (no promotable candidate) -> HOLD, never a phantom ADVANCE', realPlan.recommendation === 'HOLD');
  check('projections agree with the full plan', (await engine.readiness(0, 30, AT)).recommendation === realPlan.recommendation && (await engine.timeline(0, 30, AT)).timeline.length === realPlan.timeline.length);
  ...
  check('the real engine is deterministic in its derived content (window is a clock input)', normalize(await engine.plan(0, 30, AT)) === normalize(realPlan));
