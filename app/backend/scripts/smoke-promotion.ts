...
function rec(over: Partial<GovernanceRecommendation> = {}): GovernanceRecommendation {
  return {
    ...
    evidence: {
      pairedScans: 120,
      top1Delta: 0.08,
      top1DeltaCi: { low: 0.03, high: 0.13 },
      mcNemarZ: 3.1,
      generalizesAcrossModalities: true,
      generalizesAcrossUsers: true,
      latencyDeltaMs: -20,
      tokenDeltaPerScan: 50,
      challengerAvailability: 0.99,
      incumbentDrift: 'STABLE',
    },
...
const gates = (over: Record<string, 'PASS' | 'FAIL' | 'NOT_APPLICABLE'> = {}): GatesReport => ({
  ...
  gates: [
    ...
    { id: 'PROVIDER_READY', status: over.PROVIDER_READY ?? 'PASS', reasons: ['muestra suficiente'], evidence: {} },
    {
      ...
      status: over.ROLLBACK_REQUIRED ?? 'NOT_APPLICABLE',
      reasons: ['auto-accept off'],
      evidence: {},
    },
...
const health = (over: Partial<HealthReport> = {}): HealthReport => ({
  ...
  acceptanceRate: 0.8,
  undoRate: 0.05,
  manualFallbackRate: 0.1,
  providerFailureRate: 0.03,
  meanLatencyMs: 900,
  p50LatencyMs: 850,
  meanScanConfidence: 0.8,
  calibration: { currentEce: 0.1, previousEce: 0.11, drift: 0.01 },
  promotion: { status: 'PROMOTE_CHALLENGER', detail: 'ok' },
  falsePositives: 0,
  falseNegatives: 1,
  ...over,
});
const status = (): RolloutStatus => ({
  contractVersion: 1,
  window: W,
  generatedFor: { activeProviderId: 'alpha', autoAcceptEnabled: false },
  global: { stage: 'SHADOW', reasons: ['sombra'] },
  perModality: [],
  perProvider: [],
});
...
async function main() {
  ...
  check(
    'PROMOTE + no blocking gate + risk LOW -> READY, no blockers',
    ready.readiness === 'READY' && ready.blockingReasons.length === 0,
  );
  check('a READY plan enters the ladder at 5%', ready.rolloutPercent === 5);

  const notPromote = buildPromotionPlan(
    rec({ action: 'HOLD', reasons: ['no generaliza'] }),
    risk('LOW'),
    gates(),
    health(),
    status(),
  ...
  check(
    'Governance says HOLD -> BLOCKED, quoting Governance verbatim',
    notPromote.readiness === 'BLOCKED' &&
      notPromote.blockingReasons[0].includes('HOLD') &&
      notPromote.blockingReasons[0].includes('no generaliza'),
  );
  ...
  check(
    'a failing PROVIDER_READY gate blocks even a PROMOTE',
    gateFail.readiness === 'BLOCKED' && gateFail.blockingReasons.some((r) => r.includes('PROVIDER_READY')),
  );
  const rollbackFail = buildPromotionPlan(
    rec(),
    risk('LOW'),
    gates({ ROLLBACK_REQUIRED: 'FAIL' }),
    health(),
    status(),
    AT,
  );
  check(
    'a firing ROLLBACK_REQUIRED gate blocks promotion',
    rollbackFail.readiness === 'BLOCKED' && rollbackFail.blockingReasons.some((r) => r.includes('ROLLBACK_REQUIRED')),
  );
  const highRisk = buildPromotionPlan(
    rec(),
    risk('HIGH', [{ dimension: 'USER', level: 'HIGH', evidence: ['undo 50%'] }]),
    gates(),
    health(),
    status(),
    AT,
  ...
  check(
    'HIGH overall risk blocks, naming the worst dimension',
    highRisk.readiness === 'BLOCKED' && highRisk.blockingReasons.some((r) => r.includes('USER')),
  );
  const noCandidate = buildPromotionPlan(
    rec({ challengerId: null, action: 'REQUIRE_MORE_DATA' }),
    risk('LOW'),
    gates(),
    health(),
    status(),
  ...
  check(
    'statistical evidence is Governance verbatim',
    ready.statisticalEvidence.pairedScans === 120 &&
      ready.statisticalEvidence.mcNemarZ === 3.1 &&
      ready.statisticalEvidence.top1DeltaCi?.low === 0.03,
  );
  check(
    'risk is the Risk Engine summarized, never re-assessed',
    ready.estimatedRisk.overall === 'LOW' && ready.estimatedRisk.dimensions[0].dimension === 'TECHNICAL',
  );
  check(
    'confidence is a LABEL over the CI width, not a recomputation (0.10 wide -> MEDIA)',
    ready.confidence === 'MEDIA',
  );
  check(
    'a tight CI reads ALTA',
    buildPromotionPlan(
      rec({ evidence: { ...rec().evidence, top1DeltaCi: { low: 0.06, high: 0.09 } } }),
      risk('LOW'),
      gates(),
      health(),
      status(),
  ...
  check(
    'no CI -> SIN_EVIDENCIA',
    buildPromotionPlan(
      rec({ evidence: { ...rec().evidence, top1DeltaCi: null } }),
      risk('LOW'),
      gates(),
      health(),
      status(),
  ...
  check(
    'every rung has advance/stop/rollback conditions — never a naked percent',
    ready.rolloutStrategy.every(
      (s) => s.advanceConditions.length > 0 && s.stopConditions.length > 0 && s.rollbackConditions.length > 0,
    ),
  );
  check(
    'advance conditions quote the health thresholds by their owning constants',
    ready.rolloutStrategy[0].advanceConditions.some((c) => c.includes('undo')),
  );
  check('total duration is the sum of the ladder (24+24+48+48+72=216h)', ready.estimatedDurationHours === 216);
  check(
    'rollback criteria are the de-duplicated union of the ladder',
    ready.rollbackCriteria.length > 0 && ready.rollbackCriteria.length === new Set(ready.rollbackCriteria).size,
  );
  ...
  check(
    'every checklist item is fully typed — no bare booleans',
    allItems.every((i) => !!i.status && !!i.explanation && !!i.severity && !!i.owner && !!i.category),
  );
  check(
    'all six categories are represented across the checklists',
    new Set(allItems.map((i) => i.category)).size === 6,
    [...new Set(allItems.map((i) => i.category))].join(','),
  );
  check(
    'monitoring reflects live health: undo 5% under threshold -> PASS',
    ready.monitoringChecklist.find((i) => i.label.includes('undo') || i.label.includes('undo'))?.status !== 'FAIL',
  );
  const badHealth = buildPromotionPlan(
    rec(),
    risk('LOW'),
    gates(),
    health({ undoRate: 0.5, falsePositives: 3 }),
    status(),
    AT,
  );
  check(
    'a bad undo rate flips its monitoring item to FAIL',
    badHealth.monitoringChecklist.find((i) => i.label.toLowerCase().includes('undo'))?.status === 'FAIL',
  );
  check(
    'false positives flip the safety item to FAIL',
    badHealth.monitoringChecklist.find((i) => i.label.includes('Falsos positivos'))?.status === 'FAIL',
  );
  check(
    'validation reflects paired-scan sufficiency',
    ready.validationChecklist.find((i) => i.label.includes('pareada'))?.status === 'PASS',
  );
  check(
    'insufficient paired scans -> validation FAIL',
    buildPromotionPlan(
      rec({ evidence: { ...rec().evidence, pairedScans: 5 } }),
      risk('LOW'),
      gates(),
      health(),
      status(),
      AT,
    ).validationChecklist.find((i) => i.label.includes('pareada'))?.status === 'FAIL',
  );
  check(
    'approval is PENDING when READY, FAIL on the safety gate when BLOCKED',
    ready.approvalChecklist.every((i) => i.status === 'PENDING') &&
      notPromote.approvalChecklist.find((i) => i.category === 'SAFETY')?.status === 'FAIL',
  );

  console.log('\n── V4.2: EXECUTION + ROLLBACK STEPS (ordered, owned, descriptive only) ──');
  check(
    'execution steps are ordered 1..N with owners',
    ready.executionSteps.every((s, i) => s.order === i + 1 && !!s.owner),
  );
  check(
    'execution names the candidate and warns about calibration reset',
    ready.executionSteps.some((s) => s.detail.includes('beta')) &&
      ready.executionSteps.some((s) => s.detail.includes('calibración')),
  );
  check(
    'rollback steps restore the incumbent and preserve evidence',
    ready.rollbackSteps.some((s) => s.detail.includes('alpha')) &&
      ready.rollbackSteps.some((s) => s.detail.includes('append-only')),
  );

  console.log('\n── V4.2: DETERMINISM + IDEMPOTENCY ──');
  check(
    'same verdicts + same timestamp -> byte-identical plan',
    JSON.stringify(buildPromotionPlan(rec(), risk('LOW'), gates(), health(), status(), AT)) === JSON.stringify(ready),
  );
  check(
    'generatedAt is an INPUT (reproducible), stamped on the plan',
    ready.generatedAt === AT && ready.version === PROMOTION_PLAN_VERSION,
  );
  ...
  check(
    ...
    plan.version === PROMOTION_PLAN_VERSION && plan.generatedAt === AT,
  );
  check(
    'no shadow evidence -> NOT_APPLICABLE / REQUIRE_MORE_DATA, never a phantom PROMOTE',
    plan.readiness !== 'READY' && plan.decision !== 'PROMOTE',
  );
  check(
    'the projections agree with the full plan (same underlying build)',
    (await engine.readiness(30, AT)).readiness === plan.readiness &&
      (await engine.rollback(30, AT)).rollbackCriteria.length === plan.rollbackCriteria.length,
  );
  ...
  check(
    'READ-ONLY: building the plan (governance + rollout + assembly) changed ZERO rows',
  ...
  const normalize = (p: any) =>
    JSON.stringify({ ...p, window: null, statisticalEvidence: { ...p.statisticalEvidence } });
  check(
    'the real engine is deterministic in its DERIVED content (window is a clock input, like generatedAt)',
    normalize(await engine.plan(30, AT)) === normalize(plan),
  );
