...
import { decideNudge, decidePlanAdjustment, stateToInput } from '../src/recommendations/services/recommendation-engine';
import { RecommendationInput, RecommendationReason, REASON_META } from '../src/recommendations/recommendation-reason';
...
function mkInput(
  over: Partial<RecommendationInput> & { state?: Partial<RecommendationInput['state']> } = {},
): RecommendationInput {
...
async function main() {
  ...
  const proteinGap = decideNudge(mkInput({ today: { caloriesLogged: 1000, proteinG: 40, mealsLogged: 2 } }));
  check(
    'protein gap today → PROTEIN_GAP_TODAY',
    proteinGap.reason === RecommendationReason.PROTEIN_GAP_TODAY,
    proteinGap.reason,
  );
  ...
  const onlyFlag = decideNudge(
    mkInput({
      today: { caloriesLogged: 1600, proteinG: 145, mealsLogged: 3 },
      state: { behaviorFlags: ['PROTEIN_CHRONIC_LOW'] },
    }),
  );
  check(
    'only a habit flag → PROTEIN_CHRONIC_LOW',
    onlyFlag.reason === RecommendationReason.PROTEIN_CHRONIC_LOW,
    onlyFlag.reason,
  );
  ...
  check(
    'plateau beats protein flag → PLATEAU_SUSPECTED',
    plateauWins.reason === RecommendationReason.PLATEAU_SUSPECTED,
    plateauWins.reason,
  );
  check(
    'nudge always single + typed',
    typeof plateauWins.message === 'string' &&
      plateauWins.type === 'PLAN_ADJUSTMENT' &&
      plateauWins.priority === 'HIGH',
  );

  const steady = decideNudge(mkInput({ today: { caloriesLogged: 1600, proteinG: 145, mealsLogged: 3 } }));
  ...
  const planPlateau = decidePlanAdjustment(
    mkInput({
      goal: 'lose',
      targets: { calories: 1900, proteinG: 150 },
      state: { plateauStatus: 'PLATEAU_SUSPECTED', trendStatus: 'stalled', adherenceScore: 80, weightDataPoints: 4 },
    }),
  );
  check(
    'plateau → PLAN cut -200',
    planPlateau?.reason === RecommendationReason.PLATEAU_SUSPECTED && planPlateau?.calorieAdjustment === -200,
    `${planPlateau?.reason}/${planPlateau?.calorieAdjustment}`,
  );
  ...
  check(
    'losing too fast → LOSING_TOO_FAST +100',
    fastLoss?.reason === RecommendationReason.LOSING_TOO_FAST && fastLoss?.calorieAdjustment === 100,
    `${fastLoss?.reason}/${fastLoss?.calorieAdjustment}`,
  );
  ...
  check(
    'gain stalled → GAIN_STALLED +100',
    gainStalled?.reason === RecommendationReason.GAIN_STALLED && gainStalled?.calorieAdjustment === 100,
    `${gainStalled?.reason}`,
  );
  ...
  console.log('\n── REASON META ──');
  check(
    'PLATEAU_SUSPECTED → PLAN_ADJUSTMENT/HIGH',
    REASON_META.PLATEAU_SUSPECTED.type === 'PLAN_ADJUSTMENT' && REASON_META.PLATEAU_SUSPECTED.priority === 'HIGH',
  );
  check(
    'PROTEIN_CHRONIC_LOW → BEHAVIOR/MEDIUM',
    REASON_META.PROTEIN_CHRONIC_LOW.type === 'BEHAVIOR_RECOMMENDATION' &&
      REASON_META.PROTEIN_CHRONIC_LOW.priority === 'MEDIUM',
  );
  ...
  check(
    'rollup loggingStreak = 6 (log-derived, self-healing)',
    plState.loggingStreak === 6,
    `got ${plState.loggingStreak}`,
  );
  check(
    'rollup proteinStreakDays = 6 (140 >= 90% of 150)',
    plState.proteinStreakDays === 6,
    `got ${plState.proteinStreakDays}`,
  );
  check(
    'rollup calorieStreakDays = 6 (1850 within target band)',
    plState.calorieStreakDays === 6,
    `got ${plState.calorieStreakDays}`,
  );
  const plDecision = decidePlanAdjustment(
    stateToInput({ goal: 'LOSE_FAT', state: plState, today: { caloriesLogged: 0, proteinG: 0, mealsLogged: 0 } }),
  );
  check(
    'engine (consumer) → cut -200 from rollup state',
    plDecision?.reason === RecommendationReason.PLATEAU_SUSPECTED && plDecision?.calorieAdjustment === -200,
    `${plDecision?.reason}`,
  );
  ...
  check(
    'rollup: flat but low adherence → plateau NOT suspected',
    loState.plateauStatus !== 'PLATEAU_SUSPECTED',
    loState.plateauStatus,
  );
  check(
    'rollup: non-consecutive logs → loggingStreak 0 (no frozen streak)',
    loState.loggingStreak === 0,
    `got ${loState.loggingStreak}`,
  );
  const loDecision = decidePlanAdjustment(
    stateToInput({ goal: 'LOSE_FAT', state: loState, today: { caloriesLogged: 0, proteinG: 0, mealsLogged: 0 } }),
  );
  ...
  const saved = await prisma.recommendation.create({
    data: {
      userId: plU.id,
      type: plDecision!.type,
      priority: plDecision!.priority,
      trigger: 'weight.updated',
      reason: plDecision!.reason,
      messageForUser: plDecision!.message,
      planChange: true,
      calorieAdjustment: plDecision!.calorieAdjustment,
  ...
  check(
    'snapshot: scores present',
    snap.scores.adherence !== null && snap.scores.nutrition !== null,
    `adh=${snap.scores.adherence} nut=${snap.scores.nutrition}`,
  );
  ...
  check(
    'snapshot: weekly streaks exposed (logging=6)',
    snap.weekly.loggingStreak === 6 && snap.weekly.proteinStreakDays === 6 && snap.weekly.calorieStreakDays === 6,
  );
  check(
    'snapshot: topRecommendation reason exposed',
    snap.topRecommendation?.reason === 'PLATEAU_SUSPECTED',
    snap.topRecommendation?.reason ?? 'null',
  );
  check(
    'snapshot: topRecommendation carries id + status (commit-ready)',
    typeof snap.topRecommendation?.id === 'string' && snap.topRecommendation?.status === 'PENDING',
    `${snap.topRecommendation?.status}`,
  );
  ...
  const mkRec = (over: any = {}) =>
    prisma.recommendation.create({
      data: {
        ...
        messageForUser: 'Agrega 25g de proteína al desayuno.',
        planChange: false,
  ...
  check(
    'commit: sets committedAt + a future commitExpiresAt',
    !!committed.committedAt && committed.commitExpiresAt > new Date(),
  );
  ...
  check(
    'lazy expiry: stale commitment swept to EXPIRED on read',
    sweptRow?.status === 'EXPIRED',
    sweptRow?.status ?? 'null',
  );
  ...
  check(
    'complete after window: refuses and marks EXPIRED',
    lateComplete.status === 'EXPIRED' && !!lateComplete.message,
  );

  // getActive surfaces live commitments alongside pending nudges
  const active = await recsSvc.getActive(cU.id);
  check(
    'getActive: includes PENDING + live COMMITTED only',
    active.every((r: any) => r.status === 'PENDING' || r.status === 'COMMITTED'),
    `n=${active.length}`,
  );
