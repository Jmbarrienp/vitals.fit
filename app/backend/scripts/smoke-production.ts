...
async function main() {
  ...
  check('missing DATABASE_URL is a hard error', collectEnvProblems({ JWT_SECRET: GOOD_SECRET }).errors.some((e) => e.includes('DATABASE_URL')));
  check('a non-postgres DATABASE_URL is rejected', collectEnvProblems({ ...GOOD_ENV, DATABASE_URL: 'mysql://x' }).errors.some((e) => e.includes('postgres')));
  check('missing JWT_SECRET is a hard error', collectEnvProblems({ DATABASE_URL: GOOD_ENV.DATABASE_URL }).errors.some((e) => e.includes('JWT_SECRET')));
  // THE bug this slice exists for: .env.example ships JWT_SECRET=changeme.
  check('THE PLACEHOLDER "changeme" IS REJECTED — it is public in .env.example, so every token would be forgeable', collectEnvProblems({ ...GOOD_ENV, JWT_SECRET: 'changeme' }).errors.some((e) => e.includes('forgeable')));
  check('other common placeholders are rejected too', ['secret', 'password', 'test'].every((s) => collectEnvProblems({ ...GOOD_ENV, JWT_SECRET: s }).errors.length > 0));
  check('a short secret is a hard error IN PRODUCTION', collectEnvProblems({ ...GOOD_ENV, JWT_SECRET: 'short', NODE_ENV: 'production' }).errors.some((e) => e.includes('brute force')));
  check('…but only a warning in development (local dev is not blocked)', (() => {
    const r = collectEnvProblems({ ...GOOD_ENV, JWT_SECRET: 'short', NODE_ENV: 'development' });
    return r.errors.length === 0 && r.warnings.length > 0;
  })());
  check('EVERY problem is reported at once, not one per restart', collectEnvProblems({ JWT_SECRET: 'changeme' }).errors.length >= 2);
  check('SHADOW_SAMPLE_RATE out of [0,1] is rejected', collectEnvProblems({ ...GOOD_ENV, SHADOW_SAMPLE_RATE: '5' }).errors.length === 1 && collectEnvProblems({ ...GOOD_ENV, SHADOW_SAMPLE_RATE: '0.1' }).errors.length === 0);
  check('a non-boolean AUTO_ACCEPT_ENABLED is rejected (it would silently read as false)', collectEnvProblems({ ...GOOD_ENV, AUTO_ACCEPT_ENABLED: 'yes' }).errors.length === 1);
  check('a non-integer PORT is rejected', collectEnvProblems({ ...GOOD_ENV, PORT: 'abc' }).errors.length === 1);
  check('optional switches absent -> no error (documented defaults respected)', collectEnvProblems(GOOD_ENV).errors.length === 0);
  check('production posture warnings never block a deploy', (() => {
  ...
  let threw = false;
  try { validateEnv({ JWT_SECRET: 'changeme' }); } catch { threw = true; }
  ...
  const mkHost = (captured: any) => ({
    switchToHttp: () => ({
      getResponse: () => ({ status(code: number) { captured.status = code; return this; }, json(body: any) { captured.body = body; return this; } }),
  ...
  check('a BadRequestException keeps its 400 AND its exact body', badRequest.status === 400 && JSON.stringify(badRequest.body).includes('El campo X es inválido'));
  const notFound: any = {};
  filter.catch(new NotFoundException('Comida no encontrada.'), mkHost(notFound));
  check('a NotFoundException keeps its 404 and message (no behavior change)', notFound.status === 404 && JSON.stringify(notFound.body).includes('Comida no encontrada'));
  const validationLike: any = {};
  filter.catch(new HttpException({ statusCode: 400, message: ['calories must be a number'], error: 'Bad Request' }, 400), mkHost(validationLike));
  check('a ValidationPipe-shaped body passes through byte-identical', JSON.stringify(validationLike.body.message) === JSON.stringify(['calories must be a number']));
  const unhandled: any = {};
  filter.catch(new Error('Invalid `prisma.user.findUnique()` on column User.email'), mkHost(unhandled));
  check('an UNHANDLED error becomes an opaque 500 — no Prisma internals leak', unhandled.status === 500 && !JSON.stringify(unhandled.body).includes('prisma') && unhandled.body.message === 'Internal server error');
  ...
  check('HEALTH: a healthy database reports ok with a measured latency', healthy.status === 'ok' && healthy.database.ok === true && typeof healthy.database.latencyMs === 'number');
  check('…and the ORIGINAL fields are unchanged (backward compatible)', healthy.service === 'fitness-ai-backend' && typeof healthy.timestamp === 'string' && res.statusCode === 200);
  check('…the process liveness probe stays dependency-free', typeof new AppController(prisma).live().uptimeSeconds === 'number');

  // The bug: the old endpoint was a static object and COULD NOT report illness.
  const brokenPrisma = { ping: async () => { throw new Error('ECONNREFUSED'); } } as any;
  ...
  check('HEALTH: an unreachable database reports DEGRADED, not ok', degraded.status === 'degraded' && degraded.database.ok === false);
  ...
  const hangRes: any = { statusCode: 200, status(c: number) { this.statusCode = c; return this; } };
  const hung = await new AppController(hangingPrisma).health(hangRes);
  check('HEALTH: a hanging database times out instead of hanging the probe', hung.status === 'degraded' && hung.database.error === 'HEALTH_PROBE_TIMEOUT');
  ...
  (coaching as any).build = async (...args: any[]) => { builds++; return realBuild(...(args as [string, any])); };
  ...
  check('PERFORMANCE: a Copilot session builds the CoachingContext EXACTLY ONCE (was 3x)', builds === 1, `${builds} build(s)`);
  ...
  check('COMPATIBILITY: a shared context yields a byte-identical plan', JSON.stringify(stripTime(planShared)) === JSON.stringify(stripTime(planOwn)));
  ...
  check('COMPATIBILITY: a shared context yields a byte-identical meal plan', JSON.stringify(stripTime(mealShared)) === JSON.stringify(stripTime(mealOwn)));
  check('COMPATIBILITY: the Copilot session is unchanged by the optimization', s1.currentFocus.area.length > 0 && d1.focus.area === s1.currentFocus.area);
  ...
  try { await disposable.onModuleDestroy(); } catch { secondDestroyThrew = true; }
