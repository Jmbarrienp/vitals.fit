...
function mkSession(over: Partial<CopilotSession> = {}): CopilotSession {
  return {
    meta: {
      contract: 'vitals-fit.copilot-session',
      version: 1,
      generatedAt: AT,
      consumes: { coachingContext: 1, planner: 1, mealPlanner: 1 },
    },
    ...
    currentPlan: {
      posture: 'STABLE',
      headlineCode: 'HOLD_PLAN',
      headlineExplanation: 'mantener el plan actual',
      decisions: 1,
      reviewWindowDays: 14,
    },
    activeCommitments: [],
    ...
    recentProgress: {
      ...
      adherence7d: 82,
      loggingStreakDays: 6,
      lastWeek: { adherenceScore: 78, biggestImprovement: 'ADHERENCE_IMPROVED' },
    },
    mealSuggestions: [
    ...
    nextAction: {
      source: 'WEEKLY_COACH',
      action: 'Sube tu proteína del desayuno a 30g',
      reason: 'la próxima acción del coach es la más fundamentada',
    },
    pendingQuestions: [],
    confidence: 'ALTA',
...
async function main() {
  ...
  check(
    'the projection exposes ONE focus object, not a list',
    typeof daily.focus === 'object' && !Array.isArray(daily.focus) && !!daily.focus.area,
  );
  check('the focus area comes from the runtime verbatim', daily.focus.area === 'MAINTAIN');
  check(
    "the WHY is the runtime's evidence-based reason, verbatim",
    daily.focus.why === 'todo en verde — proteger la racha es el mejor movimiento',
  );
  check(
    'the title is presentation copy the SCREEN does not have to know',
    daily.focus.title === 'Vas bien — mantén el ritmo',
  );
  const logging = projectDailySession(mkSession({ currentFocus: { area: 'LOGGING', reason: 'sin comidas hoy' } }), AT);
  check(
    'a different focus area yields a different title, still exactly one',
    logging.focus.title === 'Retoma tu registro' && typeof logging.focus === 'object',
  );

  console.log("\n── V5.1: TODAY'S PLAN (priority first, deduplicated) ──");
  check(
    'the priority action is ALWAYS position 1',
    daily.todaysPlan.actions[0].kind === 'PRIORITY' &&
      daily.todaysPlan.actions[0].text.includes('proteína del desayuno'),
  );
  check('…and it carries its owning source', daily.todaysPlan.actions[0].source === 'WEEKLY_COACH');
  const withCommitment = projectDailySession(
    mkSession({
      activeCommitments: [{ message: 'Comer 30g de proteína al desayuno', reason: 'P', expiresAt: '2026-07-18' }],
    }),
    AT,
  );
  check(
    'a live pledge is added as a second concrete step',
    withCommitment.todaysPlan.actions.length === 2 && withCommitment.todaysPlan.actions[1].kind === 'COMMITMENT',
  );
  const dupCommitment = projectDailySession(
    mkSession({
  ...
  check(
    'NO DUPLICATION: a pledge identical to the priority is not repeated',
    dupCommitment.todaysPlan.actions.length === 1,
  );
  const withVision = projectDailySession(
    mkSession({
      visionSuggestions: [
        { source: 'VISION', text: 'Registra con la cámara — menos fricción', code: 'VISION_CAPTURE_AFFORDANCE' },
      ],
    }),
    AT,
  );
  check(
    'the logging affordance becomes a LOG step when the runtime offered it',
    withVision.todaysPlan.actions.some((a) => a.kind === 'LOG'),
  );

  console.log('\n── V5.1: SECTIONS (each traceable to its owner) ──');
  check(
    "meals are the Meal Planner's, verbatim",
    daily.meals.items.length === 2 && daily.meals.items[0].name === 'Pollo con arroz',
  );
  const noMeals = projectDailySession(
    mkSession({
      mealSuggestions: [],
      silenced: [
        { module: 'MEAL_PLANNER', reason: 'el foco es reparar el registro — sugerir comidas antes de eso es ruido' },
      ],
    }),
    AT,
  );
  check(
    'an empty meal list QUOTES the coordination audit instead of inventing a reason',
    noMeals.meals.note?.includes('reparar el registro') === true,
  );
  check(
    'commitments project with their expiry; empty state has its own note',
    daily.commitments.active.length === 0 &&
      !!daily.commitments.note &&
      withCommitment.commitments.active[0].expiresAt === '2026-07-18',
  );
  check('progress headline is copy over the ALREADY-COMPUTED trend', daily.progress.headline === 'Vas en camino');
  check(
    'progress metrics are pre-formatted — the screen prints, never computes',
    daily.progress.metrics.some((m) => m.value === '1200 / 2000 kcal') &&
      daily.progress.metrics.some((m) => m.value === '82%'),
  );
  check(
    "last week's score appears only when the review produced one",
    daily.progress.metrics.some((m) => m.label === 'Semana pasada') &&
      !projectDailySession(
        mkSession({ recentProgress: { trend: 'on_track', adherence7d: 80, loggingStreakDays: 3, lastWeek: null } }),
        AT,
      ).progress.metrics.some((m) => m.label === 'Semana pasada'),
  );
  check(
    'the Vision CTA appears ONLY when the runtime decided it lowers friction',
    daily.visionCta === null && withVision.visionCta?.target === 'LOG',
  );
  check('confidence is inherited, not recomputed', daily.confidence === 'ALTA');

  console.log('\n── V5.1: PURE PROJECTION + DETERMINISM ──');
  check(
    'the source contract version is pinned (provenance)',
    daily.meta.source.copilotSessionVersion === 1 && daily.meta.version === DAILY_COPILOT_CONTRACT_VERSION,
  );
  check('generatedAt is an INPUT — reproducible', daily.meta.generatedAt === AT);
  check(
    'same session + same timestamp -> byte-identical daily',
    JSON.stringify(projectDailySession(mkSession(), AT)) === JSON.stringify(daily),
  );
  const trends = ['on_track', 'stalled', 'regressing', 'insufficient_data'];
  check(
    'every trend the platform can emit has copy (no unlabeled state)',
    trends.every((t) => {
      const d = projectDailySession(
        mkSession({ recentProgress: { trend: t, adherence7d: null, loggingStreakDays: 0, lastWeek: null } }),
        AT,
      );
  ...
  check(
    'an unknown trend degrades to a safe headline instead of crashing',
    projectDailySession(
      mkSession({
        recentProgress: { trend: 'martian', adherence7d: null, loggingStreakDays: 0, lastWeek: null },
      } as any),
      AT,
    ).progress.headline === 'Tu progreso',
  );
  check(
    'adherence with no data reads "sin datos", never 0%',
    projectDailySession(
      mkSession({ recentProgress: { trend: 'on_track', adherence7d: null, loggingStreakDays: 0, lastWeek: null } }),
      AT,
    ).progress.metrics.some((m) => m.value === 'sin datos'),
  );
  ...
  check(
    'the runtime produces a versioned daily session over the real graph',
    d1.meta.version === DAILY_COPILOT_CONTRACT_VERSION && d1.meta.generatedAt === AT,
  );
  check(
    'EXACTLY ONE focus, with a title and a why',
    !!d1.focus.area && d1.focus.title.length > 0 && d1.focus.why.length > 0,
  );
  check(
    'a fresh user (no meals) focuses on logging and gets the Vision CTA',
    d1.focus.area === 'LOGGING' && d1.visionCta !== null,
  );
  check(
    '…and the meal list is empty WITH the audited reason (Meal Planner silenced)',
    d1.meals.items.length === 0 && !!d1.meals.note,
  );
  check(
    'the plan always has at least the priority action',
    d1.todaysPlan.actions.length >= 1 && d1.todaysPlan.actions[0].kind === 'PRIORITY',
  );
  check(
    'no duplicate action text within the plan',
    new Set(d1.todaysPlan.actions.map((a) => a.text.toLowerCase())).size === d1.todaysPlan.actions.length,
  );
  check(
    'progress carries real metrics from the state engine',
    d1.progress.metrics.length >= 4 && d1.progress.headline.length > 0,
  );
  check(
    'the coordination session stays available alongside the daily one',
    (await runtime.session(user.id, AT)).silenced.length > 0,
  );
  ...
  check(
    'ZERO WRITES: the daily projection added no rows',
