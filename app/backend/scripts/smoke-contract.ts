...
async function main() {
  ...
  await prisma.userProfile.create({ data: {
    userId: user.id, name: 'Contract Tester', age: 30, weightKg: 80, heightCm: 178,
    sex: 'MALE', activityLevel: 'MODERATE', fitnessLevel: 'BEGINNER',
  } });
  ...
  const logDay = (date: Date, proteinG: number) =>
    prisma.dailyLog.create({ data: {
      userId: user.id, date, caloriesLogged: 2000, proteinG, planFollowed: true, adherencePct: 1.0,
      loggedMeals: { create: [
  ...
  await prisma.dailyLog.create({ data: {
    userId: user.id, date: localToday, caloriesLogged: 700, proteinG: 50,
    loggedMeals: { create: [{ mealType: 'LUNCH' as const, totalCalories: 700, totalProteinG: 50, totalCarbsG: 70, totalFatG: 20, name: 'Pollo con arroz' }] },
  } });

  for (const [d, kg] of [[addDaysUTC(weekB, 0), 80.0], [addDaysUTC(weekB, 3), 79.7], [addDaysUTC(weekC, 0), 79.3], [addDaysUTC(weekC, 3), 79.0]] as const) {
  ...
  await prisma.recommendation.create({ data: {
    userId: user.id, type: 'BEHAVIOR_RECOMMENDATION', priority: 'MEDIUM', trigger: 'meal.logged',
    reason: 'PROTEIN_CHRONIC_LOW', messageForUser: 'Sube 25g de proteina al desayuno.',
    status: 'COMPLETED', createdAt: addDaysUTC(weekB, 1), committedAt: addDaysUTC(weekB, 2),
    commitExpiresAt: addDaysUTC(weekC, 2), completedAt: addDaysUTC(weekC, 2),
  } });
  await prisma.recommendation.create({ data: {
    userId: user.id, type: 'BEHAVIOR_RECOMMENDATION', priority: 'LOW', trigger: 'meal.logged',
    reason: 'BREAKFAST_SKIPPED', messageForUser: 'Desayuna con proteina antes de las 10am.',
    status: 'COMMITTED', createdAt: addDaysUTC(currentWeekStart, 0), committedAt: addDaysUTC(currentWeekStart, 0),
  ...
  check('meta: name + version pinned', full.meta.contract === 'vitals-fit.coaching-context' && full.meta.version === COACHING_CONTRACT_VERSION, `v${full.meta.version}`);
  check('user mapped to neutral vocabulary', full.user.goal === 'lose' && full.user.persona === 'beginner' && full.user.sex === 'male');
  check('targets from goal', full.targets.calories === 2000 && full.targets.proteinG === 150);
  check('today: 1 meal, 700 kcal, meal name present', full.today.mealsLogged === 1 && full.today.recentMeals[0]?.calories === 700);
  check('currentState: streaks + weight + scores present', full.currentState.streaks.loggingDays >= 0 && full.currentState.weight.dataPoints === 4 && full.currentState.adherenceScore !== null);
  check('history: 2 completed weeks, newest first', full.history.weeks.length === 2 && full.history.weeks[0].weekStart > full.history.weeks[1].weekStart, `${full.history.weeks.length}`);
  check('history week carries primaryIssue code', full.history.weeks[1].primaryIssue === 'PROTEIN_CHRONIC_LOW', `${full.history.weeks[1].primaryIssue}`);
  check('review present with resolved follow-up', !!full.review && full.review.followUp.resolved.some((i) => i.issue === 'PROTEIN_CHRONIC_LOW' && i.intervention === 'INTERVENED'));
  check('review nextPriority RESOLVED_NEXT', full.review?.nextPriority?.basis === 'RESOLVED_NEXT', `${full.review?.nextPriority?.basis}`);
  check('commitments: 1 active with absolute expiry date', full.commitments.active.length === 1 && /^\d{4}-\d{2}-\d{2}$/.test(full.commitments.active[0].expiresAt));

  // ── DECOUPLING: no schema leakage ──
  console.log('\n── DECOUPLING ──');
  const json = JSON.stringify(full);
  check('no UUIDs anywhere in the payload', !/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i.test(json));
  ...
  check('no raw-log tables leaked (dailyLog/loggedMeal keys absent)', !json.includes('dailyLog') && !json.includes('loggedMeal'));
  ...
  check("depth 'today': today/state/commitments still present", daily.today.mealsLogged === 1 && daily.currentState.adherenceScore !== null && daily.commitments.active.length === 1);
  ...
  check('renderer omits generatedAt (pure of the clock)', !prompt.includes(full.meta.generatedAt) && !prompt2.includes(full2.meta.generatedAt));
  check('renders all sections', ['PERFIL', 'OBJETIVOS', 'HOY', 'ESTADO', 'HISTORIA SEMANAL', 'REVIEW', 'COMPROMISOS ACTIVOS'].every((s) => prompt.includes(s)), prompt.split('\n')[0]);
  check('renders structured codes verbatim', prompt.includes('PROTEIN_CHRONIC_LOW') && prompt.includes('RESOLVED_NEXT'));
  ...
  check('contextToInput feeds the deterministic engine', typeof decided.message === 'string' && !!decided.reason, decided.reason);
