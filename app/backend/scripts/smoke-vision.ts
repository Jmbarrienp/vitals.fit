...
} from '../src/vision/providers/claude-vision.prompt';
...
import {
  ...
  parseBasis,
  labelCompleteness,
} from '../src/vision/pipeline/label-parser';
...
async function main() {
  ...
  const withResults = matchDetection({ label: 'pollo', labelConfidence: 0.9 }, [
    {
      id: 'f1',
  ...
  check(
    'top search result becomes the primary match',
    withResults.foodItemId === 'f1' && withResults.alternates[0]?.foodItemId === 'f2',
  );
  check('favorite + common food scores near 1.0', withResults.matchScore === 1);

  console.log('\n── PORTION (pure) ──');
  const providerHint = estimatePortion(
    { label: 'x', labelConfidence: 0.8, portionHint: { grams: 180, confidence: 0.7 } },
    90,
  );
  check(
    'PROVIDER_ESTIMATE wins when a hint exists',
    providerHint.method === 'PROVIDER_ESTIMATE' && providerHint.grams === 180,
  );
  const servingDefault = estimatePortion({ label: 'x', labelConfidence: 0.8 }, 90);
  check(
    'SERVING_DEFAULT used when no provider hint',
    servingDefault.method === 'SERVING_DEFAULT' && servingDefault.grams === 90,
  );
  ...
  check(
    'bandFor is a pure threshold function',
    bandFor(0.8) === 'HIGH' && bandFor(0.5) === 'MEDIUM' && bandFor(0.1) === 'LOW',
  );
  check('UX mode: HIGH -> CONFIRM', deriveUxMode('HIGH', null, 2) === 'CONFIRM');
  check('UX mode: MEDIUM -> REVIEW', deriveUxMode('MEDIUM', null, 2) === 'REVIEW');
  check(
    'UX mode: LOW / no-detections / failure -> FALLBACK',
    deriveUxMode('LOW', null, 2) === 'FALLBACK' &&
      deriveUxMode('HIGH', null, 0) === 'FALLBACK' &&
      deriveUxMode('HIGH', 'PROVIDER_ERROR', 2) === 'FALLBACK',
  );
  ...
  check(
    'no userId or raw-log keys in candidate output',
    !JSON.stringify(run1).includes('userId') && !JSON.stringify(run1).includes('dailyLog'),
  );
  ...
  const badDetections = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 5,
  ...
  const badConfidence = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 5,
    detections: [{ label: 'pollo', labelConfidence: 5 }],
  });
  check(
    'out-of-range labelConfidence -> invalid with a specific error',
    badConfidence.valid === false && (badConfidence as any).errors.some((e: string) => e.includes('labelConfidence')),
  );
  ...
  const stub2: VisionProvider = {
    ...
    async recognize() {
      return { providerId: 'stub2', model: 'm', providerVersion: '1', detections: [], latencyMs: 0 };
    },
  };
  ...
  let swapThrew = false;
  try {
    new VisionProviderRegistry(new ConfigService({ VISION_PROVIDER: 'ghost' }), [fixture]).active();
  } catch {
    swapThrew = true;
  }
  check('unknown provider throws (fails loud, not silent)', swapThrew === true);
  check(
    'fixture advertises multiFood, not barcode',
    fixture.capabilities.multiFood === true && fixture.capabilities.barcode === false,
  );
  ...
  check(
    'all executed cases are deterministic (same imageRef -> same output)',
    report.summary.allDeterministic === true,
  );
  check('latency is measured', typeof report.summary.avgLatencyMs === 'number');
  check(
    'standard plate case executed + succeeded',
    report.cases.find((c) => c.name === 'standard_plate')?.success === true,
  );
  check(
    'barcode case reports UNSUPPORTED (capability negotiation)',
    report.cases.find((c) => c.name === 'barcode_probe')?.capabilitySupported === false &&
      report.summary.unsupported === 1,
  );
  const stub2Report = await evaluateProvider(stub2, DEFAULT_EVAL_CASES);
  check(
    'a different provider runs through the SAME harness unchanged',
    stub2Report.providerId === 'stub2' && stub2Report.summary.allContractValid === true,
  );
  ...
  check(
    'determinism probe costs a 2nd call per case (paid providers can skip it)',
    probedCalls === probedCases.length * 2 && recognizeCalls === probedCases.length,
  );
  check(
    'unprobed determinism reports null — "not measured", never a false pass/fail',
    cheapReport.summary.allDeterministic === null && cheapReport.cases[0].deterministic === null,
  );
  ...
  check(
    'compareProviders runs the SAME cases across providers (the evidence for choosing one)',
    comparison.length === 2 && comparison[0].providerId === 'fixture' && comparison[1].providerId === 'stub2',
  );
  ...
  check(
    'put() returns an opaque ref, not the bytes',
    storedRef.startsWith('vision-mem://') && !storedRef.includes(PNG_1PX),
  );
  const resolved = await store.resolve(storedRef);
  check(
    'resolve() returns the bytes for a known ref',
    resolved?.base64 === PNG_1PX && resolved?.mimeType === 'image/png',
  );
  check(
    'resolve() returns null (never throws) for a foreign ref — fixture keyword refs stay valid',
    (await store.resolve('eval-chicken-plate.jpg')) === null,
  );
  ...
  try {
    await store.put(PNG_1PX, 'image/tiff');
  } catch {
    rejectedMime = true;
  }
  ...
  try {
    await store.put('A'.repeat(9_000_000), 'image/jpeg');
  } catch {
    rejectedSize = true;
  }
  ...
  check(
    'adapter implements the port unchanged (id + capabilities)',
    claudeNoKey.id === 'claude' &&
      claudeNoKey.capabilities.multiFood === true &&
      claudeNoKey.capabilities.barcode === false,
  );
  let noKeyErr = '';
  try {
    await claudeNoKey.recognize({ imageRef: 'x.jpg', source: 'PHOTO' });
  } catch (e: any) {
    noKeyErr = String(e.message);
  }
  check(
    'no key -> raises before any network call (service degrades it to manual)',
    noKeyErr.includes('VISION_PROVIDER_UNAVAILABLE'),
  );
  // A fake key builds a client but never calls out: the unresolved-image guard runs first.
  const claudeFakeKey = new ClaudeVisionProvider(
    new ConfigService({ ANTHROPIC_API_KEY: 'sk-ant-not-a-real-key' }),
    store,
  );
  let unresolvedErr = '';
  try {
    await claudeFakeKey.recognize({ imageRef: 'no-such-ref.jpg', source: 'PHOTO' });
  } catch (e: any) {
    unresolvedErr = String(e.message);
  }
  check(
    'unresolvable image -> raises BEFORE spending a provider call',
    unresolvedErr.includes('VISION_IMAGE_UNRESOLVED'),
  );
  check(
    'adapter is registered alongside the fixture (swap = 1 line)',
    new VisionProviderRegistry(new ConfigService({ VISION_PROVIDER: 'claude' }), [fixture, claudeNoKey]).active().id ===
      'claude',
  );

  console.log('\n── V2: PROMPT/SCHEMA CONTAINMENT + PARSER (pure) ──');
  check(
    'schema forbids extra keys and requires every field (structured-output rules)',
    (DETECTION_SCHEMA as any).additionalProperties === false &&
      (DETECTION_SCHEMA as any).properties.detections.items.required.includes('portionGrams'),
  );
  check(
    'system prompt forbids nutrition math (separation of knowledge)',
    VISION_SYSTEM_PROMPT.includes('No calcules calorías'),
  );
  ...
  check(
    'parser normalizes label + maps portion hint to the contract',
    parsed[0].label === 'pollo a la plancha' &&
      parsed[0].portionHint?.grams === 180 &&
      parsed[0].attributes?.[0] === 'homemade',
  );
  const zeroGrams = parseDetections({
    detections: [{ label: 'sopa', labelConfidence: 0.8, portionGrams: 0, portionConfidence: 0 }],
  });
  check(
    'portionGrams 0 = "cannot estimate" -> no hint, chain falls to SERVING_DEFAULT (never an invented quantity)',
    zeroGrams[0].portionHint === undefined && estimatePortion(zeroGrams[0], 120).method === 'SERVING_DEFAULT',
  );
  const hostile = parseDetections({
    detections: [
      { label: '', labelConfidence: 0.9 },
      { label: 'arroz', labelConfidence: 'nope', boundingBox: { x: 'x' } },
      'not-an-object',
      { label: 'pan', labelConfidence: 42 },
    ],
  });
  check(
    'parser is total: drops junk, never throws, clamps out-of-range confidence',
    hostile.length === 2 &&
      hostile[0].label === 'arroz' &&
      hostile[0].labelConfidence === 0 &&
      hostile[1].labelConfidence === 1,
  );
  check(
    'parser output still passes the contract validation gate',
  ...
  check(
    'parser tolerates a non-conforming payload shape',
    parseDetections({ nope: true }).length === 0 && parseDetections(null).length === 0,
  );
  ...
  check(
    'fixture resolves a known barcode deterministically',
    knownProduct.found === true && knownProduct.product?.name === 'Galletas María',
  );
  const knownProduct2 = await barcodeFixture.lookup('7501055310209');
  check(
    'same barcode -> identical result (determinism)',
    JSON.stringify(knownProduct) === JSON.stringify(knownProduct2),
  );
  const notFoundLookup = await barcodeFixture.lookup('0000000000000');
  check(
    'fixture reports not-found as found:false, not an error',
    notFoundLookup.found === false && notFoundLookup.product === null,
  );
  const genericLookup = await barcodeFixture.lookup('9999999999999');
  check(
    'an unrecognized-but-plausible barcode resolves to a generic product (demoable happy path)',
    genericLookup.found === true && genericLookup.product !== null,
  );
  const bcSwap = new BarcodeLookupProviderRegistry(new ConfigService({ BARCODE_LOOKUP_PROVIDER: 'openfoodfacts' }), [
    barcodeFixture,
    new OpenFoodFactsLookupProvider(),
  ]);
  check('config selects the active barcode provider (swap to openfoodfacts)', bcSwap.active().id === 'openfoodfacts');
  const bcDefault = new BarcodeLookupProviderRegistry(new ConfigService({}), [
    barcodeFixture,
    new OpenFoodFactsLookupProvider(),
  ]);
  check(
    'default barcode provider is openfoodfacts (free + keyless, unlike vision)',
    bcDefault.active().id === 'openfoodfacts',
  );
  let bcSwapThrew = false;
  try {
    new BarcodeLookupProviderRegistry(new ConfigService({ BARCODE_LOOKUP_PROVIDER: 'ghost' }), [
      barcodeFixture,
    ]).active();
  } catch {
    bcSwapThrew = true;
  ...
  const { candidate: bcCandidate, scanConfidence: bcConfidence } = buildBarcodeCandidate(
    cachedFood,
    { servingGrams: 30 },
    null,
  );
  check(
    'a resolved barcode always carries a real foodItemId (never a null/orphan candidate)',
    bcCandidate.foodItemId === 'f-cached',
  );
  check('barcode identity is exact -> matchScore 1, not a fuzzy rank', bcCandidate.matchScore === 1);
  check(
    'serving hint from the product feeds portion estimation (PROVIDER_ESTIMATE)',
    bcCandidate.portion.method === 'PROVIDER_ESTIMATE' && bcCandidate.portion.grams === 30,
  );
  check(
    'confidence band reflects portion uncertainty, not recognition doubt (recognition+match are both certain)',
    bcConfidence.band === 'HIGH' || bcConfidence.band === 'MEDIUM',
  );
  const { candidate: bcNoHint } = buildBarcodeCandidate(cachedFood, null, 45);
  check(
    'no product hint -> falls back to the FoodItem default serving (chain reused unchanged)',
    bcNoHint.portion.method === 'SERVING_DEFAULT' && bcNoHint.portion.grams === 45,
  );
  ...
  check(
    'tolerates units, spaces and prefixes',
    parseNumber('  8 g ') === 8 && parseNumber('<1 g') === 1 && parseNumber('about 8') === 8,
  );
  check(
    'unreadable field -> null, never a guessed number',
    parseNumber('') === null && parseNumber('---') === null && parseNumber(null as any) === null,
  );
  ...
  check(
    'US volumetric serving -> the parenthesised gram figure wins',
    JSON.stringify(parseServing('2/3 cup (55g)')) === JSON.stringify({ size: 55, unit: 'g' }),
  );
  check('metric serving', JSON.stringify(parseServing('30 g')) === JSON.stringify({ size: 30, unit: 'g' }));
  check(
    'liquid serving keeps ml (density is not assumed here)',
    JSON.stringify(parseServing('240 ml')) === JSON.stringify({ size: 240, unit: 'ml' }),
  );
  check(
    'countable serving -> unit, never faked as grams',
    JSON.stringify(parseServing('1 barra')) === JSON.stringify({ size: 1, unit: 'unit' }),
  );
  check(
    'basis detection (EU per-100g vs per-serving)',
    parseBasis('por 100 g') === 'HUNDRED_G' && parseBasis('Per serving') === 'SERVING' && parseBasis('') === null,
  );
  ...
  check(
    'US label normalizes to per-serving canonical values',
    usLabel.calories === 240 && usLabel.protein === 5 && usLabel.carbs === 46 && usLabel.fat === 3.5,
  );
  ...
  check(
    'LATAM decimal commas normalize to numbers',
    latamLabel.protein === 2.3 && latamLabel.carbs === 22.5 && latamLabel.fat === 3.6,
  );
  const euLabel = parseLabel((await usFixture.extract({ imageRef: 'eu-label.jpg' })).fields, 'fixture');
  check(
    'EU per-100g basis is converted to the 40g serving',
    euLabel.calories === 100 && euLabel.protein === 3.2 && euLabel.carbs === 24 && euLabel.fat === 3.8,
    `${euLabel.calories}kcal`,
  );
  ...
  check(
    'an unreadable nutrient is REPORTED, never invented',
    partialLabel.missingFields.includes('protein') && partialLabel.protein === 0,
  );
  check(
    'completeness reflects the missing required field',
    Math.abs(labelCompleteness(partialLabel) - 0.8) < 0.001,
    `${labelCompleteness(partialLabel)}`,
  );
  const unreadable = parseLabel((await usFixture.extract({ imageRef: 'unreadable.jpg' })).fields, 'fixture');
  check(
    'a fully unreadable label reports every field missing, throws nothing',
    unreadable.missingFields.length >= 5 && labelCompleteness(unreadable) === 0,
  );
  ...
  check(
    'same transcription -> byte-identical label (deterministic boundary over a probabilistic provider)',
    JSON.stringify(determinism1) === JSON.stringify(determinism2),
  );
  ...
  check(
    'serving size 0 -> rejected (nothing can be logged from it)',
    zeroServing.valid === false && zeroServing.errors.some((e) => e.includes('servingSize')),
  );
  ...
  check(
    'macros outweighing the serving -> rejected as physically impossible',
    impossible.valid === false && impossible.errors.some((e) => e.includes('weigh more')),
  );
  const overCap = validateNutritionLabel({ ...usLabel, calories: 99_999 });
  check(
    'a value the confirm endpoint would 400 on is rejected HERE, not at the last step',
    overCap.valid === false && overCap.errors.some((e) => e.includes('maximum')),
  );
  ...
  check(
    'a misread macro digit is caught by mass conservation (hard reject)',
    validateNutritionLabel(misreadMacro).valid === false && atwaterPlausibility(misreadMacro) < 0.6,
  );
  ...
  check(
    'a misread calorie digit is mass-legal -> caught by Atwater as a SOFT signal, not a reject',
    misreadOutcome.valid === true && misreadOutcome.plausibility < 0.6,
    `plausibility=${misreadOutcome.plausibility.toFixed(2)}`,
  );
  check(
    "0 kcal alongside real macros is inconsistent, but still the user's call",
    atwaterPlausibility({ ...usLabel, calories: 0 }) < 0.5 &&
      validateNutritionLabel({ ...usLabel, calories: 0 }).valid === true,
  );
  check(
    'plausibility never rejects a legitimately imperfect label',
    validateNutritionLabel(latamLabel).valid === true && validateNutritionLabel(euLabel).valid === true,
  );
  ...
  check(
    'a label candidate is ALWAYS a one-off — linking a catalog food would discard the printed macros',
    labelCandidate.foodItemId === null && labelCandidate.matchScore === 0,
  );
  check(
    'the serving becomes the portion',
    labelCandidate.portion.grams === 55 && labelCandidate.portion.method === 'PROVIDER_ESTIMATE',
  );
  check(
    'a clean, complete, plausible label reaches HIGH',
    labelConfidence.band === 'HIGH',
    `${labelConfidence.overall.toFixed(2)}`,
  );
  const { scanConfidence: partialConfidence } = buildLabelCandidate(partialLabel, 1);
  check(
    'a partial label drops out of HIGH -> the user reviews it',
    partialConfidence.band !== 'HIGH',
    `${partialConfidence.band}`,
  );
  const { scanConfidence: misreadConfidence } = buildLabelCandidate(usLabel, 0.2);
  check(
    'low plausibility drags confidence down even when every field was read',
    misreadConfidence.overall < labelConfidence.overall,
  );
  const { candidate: countable } = buildLabelCandidate({ ...usLabel, servingUnit: 'unit', servingSize: 1 }, 1);
  check(
    'an unconvertible serving degrades the portion method, never fakes grams',
    countable.portion.method === 'SERVING_DEFAULT' && countable.portion.confidence < 0.5,
  );

  console.log('\n── V3.2: OCR REGISTRY + PROMPT CONTAINMENT (pure) ──');
  const ocrSwap = new OCRProviderRegistry(new ConfigService({ OCR_PROVIDER: 'claude' }), [
    usFixture,
    new ClaudeOCRProvider(new ConfigService({}), new EphemeralImageStore()),
  ]);
  ...
  check(
    'default OCR provider is fixture (a real OCR call costs money, unlike barcode)',
    ocrDefault.active().id === 'fixture',
  );
  let ocrSwapThrew = false;
  try {
    new OCRProviderRegistry(new ConfigService({ OCR_PROVIDER: 'ghost' }), [usFixture]).active();
  } catch {
    ocrSwapThrew = true;
  }
  ...
  let ocrNoKeyErr = '';
  try {
    await claudeOcrNoKey.extract({ imageRef: 'x.jpg' });
  } catch (e: any) {
    ocrNoKeyErr = String(e.message);
  }
  check('no key -> raises before any network call', ocrNoKeyErr.includes('OCR_PROVIDER_UNAVAILABLE'));
  const claudeOcrFakeKey = new ClaudeOCRProvider(
    new ConfigService({ ANTHROPIC_API_KEY: 'sk-ant-not-a-real-key' }),
    new EphemeralImageStore(),
  );
  let ocrUnresolvedErr = '';
  try {
    await claudeOcrFakeKey.extract({ imageRef: 'no-such-ref.jpg' });
  } catch (e: any) {
    ocrUnresolvedErr = String(e.message);
  }
  check(
    'unresolvable image -> raises BEFORE spending a provider call',
    ocrUnresolvedErr.includes('OCR_IMAGE_UNRESOLVED'),
  );
  check(
    'schema forbids extra keys and requires every slot',
    (LABEL_SCHEMA as any).additionalProperties === false && (LABEL_SCHEMA as any).required.length === 9,
  );
  check(
    'prompt instructs transcription, forbids conversion/computation (platform owns normalization)',
    OCR_SYSTEM_PROMPT.includes('TRANSCRIBE, NO INTERPRETES') && OCR_SYSTEM_PROMPT.includes('NUNCA inventes un número'),
  );
  const extractionParsed = parseLabelExtraction({
  ...
  check(
    'extraction parser keeps slots as VERBATIM strings — it does not parse numbers',
    extractionParsed.protein === '2,3 g' && extractionParsed.calories === '132 kcal',
  );
  check(
    'extraction parser is total: missing slots -> "", out-of-range confidence clamped',
    extractionParsed.fat === '' && extractionParsed.basis === '' && extractionParsed.confidence === 1,
  );
  check(
    'extraction parser tolerates a non-conforming payload',
    parseLabelExtraction(null).calories === '' && parseLabelExtraction('nope' as any).confidence === 0,
  );
  ...
  check(
    'selectPrior: meal-type subset preferred once it has 3+ observations (lunch rice ≠ dinner rice)',
    mealTypePrior?.scope === 'MEAL_TYPE' && mealTypePrior?.median === 85,
  );
  check(
    'historyWeight grows with evidence: n=3 half-trust, n=12 dominant',
    historyWeight(3) === 0.5 && historyWeight(12) === 0.8,
  );
  ...
  check(
    'computeBias: clamped to a sane range — a 9× "bias" is a data problem, not a correction',
    computeBias([9, 9, 9, 9, 9]) === 2 && computeBias([0.01, 0.01, 0.01, 0.01, 0.01]) === 0.5,
  );
  ...
  check(
    'new user (no priors fetched) -> base estimate passes through untouched',
    newUserResolved.portion.grams === 150 &&
      newUserResolved.portion.method === 'PROVIDER_ESTIMATE' &&
      newUserResolved.portion.confidence === 0.5,
  );
  check(
    '…and the decision is still explained (single VISION signal)',
    newUserResolved.explanation.length === 1 && newUserResolved.explanation[0].source === 'VISION',
  );
  const noHistory: PortionPriorInputs = {
    userFoodGrams: [],
    userFoodMealTypeGrams: [],
    plannerExpectedGrams: null,
    visionBiasRatios: [],
  };
  const emptyResolved = resolvePortion(visionBase, noHistory);
  check(
    'priors fetched but empty -> still identical to base (regression guard)',
    emptyResolved.portion.grams === 150 && emptyResolved.portion.method === 'PROVIDER_ESTIMATE',
  );

  const strongPrior: PortionPriorInputs = {
    userFoodGrams: [165, 160, 170, 165, 168, 162, 165, 167, 164, 166],
    userFoodMealTypeGrams: [165, 160, 170, 165, 168, 162, 165, 167, 164, 166],
    plannerExpectedGrams: null,
    visionBiasRatios: [],
  };
  const wrongClaude = resolvePortion({ grams: 400, method: 'PROVIDER_ESTIMATE', confidence: 0.5 }, strongPrior);
  check(
    'wrong model estimate (400g) vs 10-log prior (~165g) -> history dominates',
    wrongClaude.portion.method === 'USER_PRIOR' &&
      Math.abs(wrongClaude.portion.grams - 165) < Math.abs(wrongClaude.portion.grams - 400),
    `${wrongClaude.portion.grams}g`,
  );
  check(
    'strong tight prior -> HIGH portion confidence (the platform KNOWS this user)',
    wrongClaude.portion.confidence >= 0.85,
  );
  const historySignal = wrongClaude.explanation.find((s) => s.source === 'USER_HISTORY');
  check(
    'explanation carries the history signal with a dominant weight',
    !!historySignal && historySignal!.weight > 0.7,
  );
  const agreeing = resolvePortion({ grams: 163, method: 'PROVIDER_ESTIMATE', confidence: 0.5 }, strongPrior);
  check(
    'model AGREEING with history -> corroboration bonus on top of the prior tier',
    agreeing.portion.confidence > 0.85,
  );

  const weakPrior: PortionPriorInputs = {
    userFoodGrams: [165, 160],
    userFoodMealTypeGrams: [],
    plannerExpectedGrams: null,
    visionBiasRatios: [],
  };
  ...
  check(
    'progressive trust: more history pulls the estimate closer to the user',
    Math.abs(wrongClaude.portion.grams - 165) < Math.abs(weakResolved.portion.grams - 165),
    `n=10 -> ${wrongClaude.portion.grams}g, n=2 -> ${weakResolved.portion.grams}g`,
  );

  const withPlan = resolvePortion(visionBase, { ...noHistory, plannerExpectedGrams: 200 });
  check(
    'planner expectation shifts the blend toward the plan',
    withPlan.portion.grams > 150 && withPlan.portion.grams < 200,
    `${withPlan.portion.grams}g`,
  );
  check(
    'planner signal has a fixed LOW weight — the plan says SHOULD, not IS',
    withPlan.explanation.find((s) => s.source === 'PLANNER')!.weight === 0.15,
  );

  const biased = resolvePortion(
    { grams: 200, method: 'PROVIDER_ESTIMATE', confidence: 0.5 },
    { ...noHistory, visionBiasRatios: [0.8, 0.8, 0.8, 0.8, 0.8, 0.8] },
  );
  ...
  check(
    'catalog default labeled CATALOG_DEFAULT, not VISION (it is knowledge, not perception)',
    servingBlend.explanation[0].source === 'CATALOG_DEFAULT',
  );
  ...
  const absurdPrior: PortionPriorInputs = {
    userFoodGrams: [5000, 5000, 5000, 5000],
    userFoodMealTypeGrams: [],
    plannerExpectedGrams: null,
    visionBiasRatios: [],
  };
  check(
    'blend clamped to plausible grams (shared 10..600 bounds)',
    resolvePortion(visionBase, absurdPrior).portion.grams <= 600,
  );
  ...
  check(
    'below-threshold restaurant scene -> dropped entirely (absence IS the fallback state)',
    deriveRestaurantContext({ ...restScene, confidence: 0.3 }, null) === null,
  );
  const bareCtx = deriveRestaurantContext(restScene, null);
  check(
    'confident scene without a menu source -> context with empty candidates',
    bareCtx !== null && bareCtx!.restaurantName === 'La Esquina Criolla' && bareCtx!.menuCandidates.length === 0,
  );
  ...
  check(
    'menu sanitization: nameless dropped; implausible numbers NULLED (discarded, never repaired)',
    menuCtx!.menuCandidates.length === 2 &&
      menuCtx!.menuCandidates[1].calories === null &&
      menuCtx!.menuCandidates[1].proteinG === null &&
      menuCtx!.menuCandidates[1].carbsG === 10,
  );
  ...
  check(
    'menu candidates capped at 6 (a menu is a cue, not a catalog dump)',
    deriveRestaurantContext(restScene, { found: true, restaurantName: null, items: manyDishes })!.menuCandidates
      .length === 6,
  );
  check(
    'lookup not-found -> context still present, zero candidates',
    deriveRestaurantContext(restScene, { found: false, restaurantName: null, items: [] })!.menuCandidates.length === 0,
  );
  check(
    'name unreadable in image -> menu source canonical name fills in',
    deriveRestaurantContext(
      { ...restScene, restaurantName: null },
      { found: true, restaurantName: 'Sucursal Centro', items: [] },
    )!.restaurantName === 'Sucursal Centro',
  );
  check(
    'determinism: same inputs -> identical context',
    JSON.stringify(deriveRestaurantContext(restScene, null)) ===
      JSON.stringify(deriveRestaurantContext(restScene, null)),
  );

  console.log('\n── V3.4: SCENE VALIDATION GATE (strip the cue, never the scan) ──');
  const sceneOk = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 1,
  ...
  check(
    'a valid scene passes the gate intact',
    sceneOk.valid === true && (sceneOk.result as any).scene?.restaurantName === 'La Esquina Criolla',
  );
  const sceneBad = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 1,
    detections: [],
    scene: { setting: 'DISCO', confidence: 7 },
  });
  check(
    'a malformed scene is STRIPPED — the scan is never hostage to an optional cue',
    sceneBad.valid === true && (sceneBad.result as any).scene === undefined,
  );
  check(
    'pre-V3.4 providers (no scene at all) validate unchanged',
    validateRecognitionResult({ providerId: 'x', model: 'm', providerVersion: '1', latencyMs: 1, detections: [] })
      .valid === true,
  );
  const usageOk = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 1,
  ...
  check(
    'V3.5: valid usage telemetry passes the gate intact',
    usageOk.valid === true && (usageOk.result as any).usage?.inputTokens === 10,
  );
  const usageBad = validateRecognitionResult({
    providerId: 'x',
    model: 'm',
    providerVersion: '1',
    latencyMs: 1,
  ...
  check(
    'V3.5: malformed usage is STRIPPED — a billing counter never costs a proposal',
    usageBad.valid === true && (usageBad.result as any).usage === undefined,
  );
  ...
  check(
    "default menu provider is 'none' — production stays inert until a real source exists",
    defaultMenuRegistry.active().id === 'none',
  );
  check(
    'null provider always answers not-found',
    (await noneProvider.lookup({ restaurantName: 'X', category: null, detectionLabels: [] })).found === false,
  );
  const fixtureMenu = await menuFixture.lookup({
    restaurantName: 'La Esquina Criolla',
    category: null,
    detectionLabels: ['pollo'],
  });
  check(
    'fixture menu answers a known restaurant with published data',
    fixtureMenu.found === true && fixtureMenu.items.length === 4 && fixtureMenu.items[0].calories === 620,
  );
  check(
    'a dish without published nutrition carries nulls — never an invented number',
    fixtureMenu.items[3].calories === null && fixtureMenu.items[3].proteinG === null,
  );
  check(
    'unknown restaurant -> not-found (a valid, handled outcome)',
    (await menuFixture.lookup({ restaurantName: 'Ghost Diner', category: null, detectionLabels: [] })).found === false,
  );
  check(
    'config swaps the menu provider',
    new RestaurantMenuProviderRegistry(new ConfigService({ RESTAURANT_MENU_PROVIDER: 'fixture' }), [
      noneProvider,
      menuFixture,
    ]).active().id === 'fixture',
  );
  ...
  try {
    new RestaurantMenuProviderRegistry(new ConfigService({ RESTAURANT_MENU_PROVIDER: 'ghost' }), [
      noneProvider,
    ]).active();
  } catch {
    menuSwapThrew = true;
  ...
  const parsedScene = parseScene({
    scene: { setting: 'RESTAURANT', confidence: 0.9, restaurantName: '', category: 'tacos' },
  });
  check(
    'parseScene: vendor sentinels ("") become nulls — nothing downstream sees a sentinel',
    parsedScene?.setting === 'RESTAURANT' && parsedScene?.restaurantName === null && parsedScene?.category === 'tacos',
  );
  check(
    'parseScene: total — malformed payload yields undefined, never a throw',
    parseScene(null) === undefined && parseScene({ scene: 'nope' }) === undefined,
  );
  const weirdScene = parseScene({ scene: { setting: 'SPACESHIP', confidence: 9, restaurantName: 7, category: null } });
  check(
    'parseScene: unknown setting collapses to UNKNOWN, confidence clamped, bad name dropped',
    weirdScene?.setting === 'UNKNOWN' && weirdScene?.confidence === 1 && weirdScene?.restaurantName === null,
  );
  ...
  const barcodeRegistry = new BarcodeLookupProviderRegistry(new ConfigService({ BARCODE_LOOKUP_PROVIDER: 'fixture' }), [
    new FixtureBarcodeLookupProvider(),
    new OpenFoodFactsLookupProvider(),
  ]);
  const ocrRegistry = new OCRProviderRegistry(new ConfigService({ OCR_PROVIDER: 'fixture' }), [
  ...
  const menuRegistry = new RestaurantMenuProviderRegistry(new ConfigService({ RESTAURANT_MENU_PROVIDER: 'fixture' }), [
    new NullRestaurantMenuProvider(),
    new FixtureRestaurantMenuProvider(),
  ]);
  ...
  const visionSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  events.on('meal.logged', () => {
    mealLoggedFired = true;
  });
  let proposedEvt = false,
    confirmedEvt = false,
    failedEvt = false;
  events.on(VISION_EVENTS.PROPOSED, () => {
    proposedEvt = true;
  });
  events.on(VISION_EVENTS.CONFIRMED, () => {
    confirmedEvt = true;
  });
  events.on(VISION_EVENTS.FAILED, () => {
    failedEvt = true;
  });
  ...
  check(
    'scan proposes multiple candidates (multi-food)',
    proposal.status === 'PROPOSED' && proposal.candidates.length === 3,
    `${proposal.candidates.length}`,
  );
  check(
    'first candidate matched to the catalog Pollo',
    proposal.candidates[0].foodItemId === pollo.id,
    `${proposal.candidates[0].displayName}`,
  );
  check('scan-level confidence present with a band', ['HIGH', 'MEDIUM', 'LOW'].includes(proposal.scanConfidence.band));
  check(
    'proposal carries a backend-decided UX mode (V1)',
    ['CONFIRM', 'REVIEW', 'FALLBACK'].includes(proposal.mode),
    proposal.mode,
  );
  ...
  check(
    'scan persisted as PROPOSED with provider provenance',
    persisted?.status === 'PROPOSED' && persisted?.providerId === 'fixture',
  );
  check(
    'V3.5: latency telemetry persisted (the platform used to discard it); tokens null for the keyless fixture',
    (persisted as any)?.latencyMs === 1 &&
      (persisted as any)?.tokensIn === null &&
      (persisted as any)?.tokensOut === null,
  );
  ...
  check(
    'confirm returns today (LogsService.logMeal contract, unchanged)',
    Array.isArray(confirmResult.meals) && confirmResult.meals.length === 1,
  );
  ...
  check(
    'provenance stamped: source=vision + visionScanId set',
    loggedMeal?.source === 'vision' && loggedMeal?.visionScanId === proposal.scanId,
  );
  ...
  check(
    'unedited accepted candidates recorded as ACCEPTED',
    feedback.every((f: any) => f.action === 'ACCEPTED'),
    feedback.map((f: any) => f.action).join(','),
  );
  ...
  check(
    'no detections -> empty candidates + NO_DETECTIONS fallback',
    emptyProposal.candidates.length === 0 && emptyProposal.fallback.reason === 'NO_DETECTIONS',
  );
  ...
  check(
    'unmatched label -> candidate with foodItemId null',
    mysteryProposal.candidates.length === 1 && mysteryProposal.candidates[0].foodItemId === null,
  );
  const mysteryConfirm = await visionSvc.confirmScan(user.id, {
    ...
    items: [
      {
        foodItemId: null,
        customName: mysteryProposal.candidates[0].displayName,
        quantity: 1,
        calories: 200,
        proteinG: 5,
        carbsG: 20,
  ...
  check(
    'unmatched candidate still confirms as a one-off item (manual path allows this)',
    Array.isArray(mysteryConfirm.meals),
  );

  // ── provider failure -> FAILED, never a broken scan, manual fallback signaled ──
  const failRegistry = new VisionProviderRegistry(new ConfigService({ VISION_PROVIDER: 'nonexistent' }), [
    new FixtureVisionProvider(),
  ]);
  const failVisionSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    failRegistry,
  ...
  check(
    'unknown provider -> scan FAILED, not thrown to the caller',
    failedProposal.status === 'FAILED' && failedProposal.fallback.reason === 'PROVIDER_ERROR',
  );
  ...
  check(
    'validation failure recorded as the failure reason',
    !!badRow?.failureReason && badRow.failureReason.includes('INVALID_PROVIDER_RESPONSE'),
    badRow?.failureReason ?? '',
  );
  ...
  try {
    await visionSvc.confirmScan(user.id, {
      scanId: expiring.scanId,
      items: [{ foodItemId: pollo.id, quantity: 150, unit: 'g', acceptedFromCandidate: 0 }],
    });
  } catch {
  ...
  events.on(VISION_EVENTS.FALLBACK, () => {
    fallbackEvt = true;
  });
  ...
  check(
    'fallback creates NO LoggedMeal (manual flow does that)',
    mealsAfterFallback === mealsBeforeFallback,
    `${mealsBeforeFallback}->${mealsAfterFallback}`,
  );
  let fbConfirmBlocked = false;
  try {
    await visionSvc.confirmScan(user.id, {
      scanId: fbScan.scanId,
      items: [{ foodItemId: pollo.id, quantity: 150, unit: 'g', acceptedFromCandidate: 0 }],
    });
  } catch {
    fbConfirmBlocked = true;
  }
  ...
  const uploaded = await visionSvc.createScan(user.id, 'lunch-capture.jpg', 'PHOTO', {
    base64: uploadPng,
    mimeType: 'image/png',
  });
  check(
    'an uploaded photo still produces a normal proposal',
    uploaded.status === 'PROPOSED' && uploaded.candidates.length > 0,
  );
  const uploadedRow = await prisma.visionScan.findUnique({ where: { id: uploaded.scanId } });
  check(
    'the scan row stores the image STORE REF, not the caller-supplied name',
    uploadedRow!.imageRef.startsWith('vision-mem://'),
  );
  check(
    'raw image bytes never reach the database',
    !uploadedRow!.imageRef.includes(uploadPng) && !JSON.stringify(uploadedRow).includes(uploadPng),
  );
  ...
  check(
    'reference-only path (V0/V1 fixture) still works untouched — dev/CI need no vendor key',
    refOnly.status === 'PROPOSED' && refOnly.candidates.length === 3,
  );
  let badUpload = false;
  try {
    await visionSvc.createScan(user.id, 'x.jpg', 'PHOTO', { base64: uploadPng, mimeType: 'image/tiff' });
  } catch {
    badUpload = true;
  }
  const scansForUser = await prisma.visionScan.count({ where: { userId: user.id, imageRef: 'x.jpg' } });
  check(
    'a rejected image is a bad request, not a failed scan (no orphan row)',
    badUpload === true && scansForUser === 0,
  );
  ...
  check(
    'first scan of a new barcode resolves via external lookup',
    firstScan.status === 'PROPOSED' && firstScan.candidates.length === 1,
  );
  check('resolved product feeds a real catalog candidate (never orphaned)', !!firstScan.candidates[0]?.foodItemId);
  check(
    'confident barcode resolution -> CONFIRM or REVIEW, not buried in FALLBACK',
    firstScan.mode === 'CONFIRM' || firstScan.mode === 'REVIEW',
  );
  ...
  check(
    'a NEW global FoodItem was created from the lookup (source=open_food_facts, barcode stamped)',
    createdFood?.source === 'open_food_facts' &&
      createdFood?.barcode === KNOWN_BARCODE &&
      createdFood?.createdByUserId === null,
  );
  const firstScanRow = await prisma.visionScan.findUnique({ where: { id: firstScan.scanId } });
  check(
    'scan row stores the decoded barcode + a placeholder imageRef (no image exists for this modality)',
    firstScanRow?.barcodeValue === KNOWN_BARCODE && firstScanRow?.imageRef === 'barcode-scan',
  );
  check(
    'provenance stamps the real provider (openfoodfacts fixture stands in), not "local-cache" on a first-time lookup',
    firstScanRow?.providerId === 'fixture',
  );
  ...
  check(
    'scanning the same barcode again resolves to the SAME FoodItem (no duplicate row)',
    beforeCount === 1 && afterCount === 1 && secondScan.candidates[0]?.foodItemId === createdFoodId,
  );
  const secondScanRow = await prisma.visionScan.findUnique({ where: { id: secondScan.scanId } });
  check(
    'the repeat scan skipped the external lookup entirely (provenance = local-cache)',
    secondScanRow?.providerId === 'local-cache',
  );
  ...
  check(
    'an unregistered barcode is a valid, handled outcome — status stays PROPOSED, not FAILED',
    notFoundScan.status === 'PROPOSED' && notFoundScan.candidates.length === 0,
  );
  check(
    'fallback reason is specific and mode steers to manual search (barcode prefillable client-side)',
    notFoundScan.fallback.reason === 'BARCODE_NOT_FOUND' && notFoundScan.mode === 'FALLBACK',
  );

  console.log('\n── V3.1: PROVIDER FAILURE / OFFLINE (network error, not "not found") ──');
  const throwingBarcodeProvider = {
    id: 'throws',
    async lookup() {
      throw new Error('NETWORK_TIMEOUT');
    },
  };
  const throwingRegistry = new BarcodeLookupProviderRegistry(new ConfigService({ BARCODE_LOOKUP_PROVIDER: 'throws' }), [
    throwingBarcodeProvider,
  ]);
  const throwingSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  check(
    'a real lookup failure (offline/timeout) is distinct from not-found — scan FAILED, same degradation contract as vision',
    throwScan.status === 'FAILED' && throwScan.fallback.reason === 'PROVIDER_ERROR' && throwScan.mode === 'FALLBACK',
  );
  const unknownBarcodeRegistry = new BarcodeLookupProviderRegistry(
    new ConfigService({ BARCODE_LOOKUP_PROVIDER: 'ghost-barcode' }),
    [barcodeFixture],
  );
  const unknownBarcodeSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  check(
    'a misconfigured provider id fails the scan gracefully, never throws to the caller',
    unknownScan.status === 'FAILED' && unknownScan.mode === 'FALLBACK',
  );
  ...
  check(
    'LoggedMeal was created through the SAME write path, with barcode provenance',
    barcodeLoggedMeal?.source === 'vision' && barcodeLoggedMeal?.visionScanId === firstScan.scanId,
  );
  check(
    'confirmScan/rejectScan/getScan needed ZERO changes for barcode — the scan lifecycle is source-agnostic',
    (await prisma.visionScan.findUnique({ where: { id: firstScan.scanId } }))?.status === 'LOGGED',
  );
  ...
  check(
    'a readable label produces a normal PROPOSED proposal',
    labelScan.status === 'PROPOSED' && labelScan.candidates.length === 1,
  );
  check(
    'the proposal carries the transcribed facts for the user to edit',
    !!labelScan.label && labelScan.label!.calories === 240 && labelScan.label!.protein === 5,
  );
  ...
  check(
    'the candidate is a one-off (the label is the truth for this package)',
    labelScan.candidates[0].foodItemId === null,
  );
  const labelScanRow = await prisma.visionScan.findUnique({ where: { id: labelScan.scanId } });
  check(
    'the label persists INSIDE the proposal JSON — no new column, no new table',
    (labelScanRow!.proposal as any).label?.calories === 240 && labelScanRow!.barcodeValue === null,
  );
  check(
    'provenance stamped for eval attribution',
    labelScanRow!.providerId === 'fixture' && labelScanRow!.providerModel === 'fixture-ocr-v1',
  );
  ...
  await prisma.foodItem.create({
    data: {
      ...
      nameLower: 'honey nut cereal',
      nameNormalized: 'honey nut cereal',
      nameAliases: [],
      caloriesPer100g: 380,
      proteinPer100g: 8,
      carbsPer100g: 84,
  ...
  check(
    'a catalog match is surfaced as an ALTERNATE via the reused FoodService.search',
    matchedLabelScan.candidates[0].alternates.length > 0 &&
      matchedLabelScan.candidates[0].alternates[0].displayName === 'Honey Nut Cereal',
  );
  check(
    '...but the primary candidate STAYS a one-off — the printed macros are not overridden by the catalog',
    matchedLabelScan.candidates[0].foodItemId === null && matchedLabelScan.label!.calories === 240,
  );
  ...
  check(
    'a partially readable label still PROPOSES (editable), never fails outright',
    partialScan.status === 'PROPOSED' && !!partialScan.label,
  );
  check(
    'the missing field is reported so mobile opens it for editing',
    partialScan.label!.missingFields.includes('protein'),
  );
  ...
  check(
    'a physically impossible label FAILS the validation gate — never proposed to the user',
    impossibleScan.status === 'FAILED' && impossibleScan.mode === 'FALLBACK',
  );
  const impossibleRow = await prisma.visionScan.findUnique({ where: { id: impossibleScan.scanId } });
  check(
    'the validation failure is recorded for audit',
    impossibleRow!.failureReason?.includes('OCR_INVALID_LABEL') === true,
  );
  const unreadableScan = await visionSvc.createLabelScan(user.id, 'unreadable-label.jpg');
  check(
    'an unreadable label degrades to manual (serving 0 cannot be logged)',
    unreadableScan.status === 'FAILED' && unreadableScan.mode === 'FALLBACK',
  );
  const throwingOcrRegistry = new OCRProviderRegistry(new ConfigService({ OCR_PROVIDER: 'throws' }), [
    {
      id: 'throws',
      async extract() {
        throw new Error('NETWORK_TIMEOUT');
  ...
  const throwingOcrSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  check(
    'an OCR provider failure degrades to manual, never throws to the caller',
    ocrFailScan.status === 'FAILED' && ocrFailScan.fallback.reason === 'PROVIDER_ERROR',
  );
  ...
  const labelMeal = await prisma.loggedMeal.findFirst({
    where: { visionScanId: labelScan.scanId },
    include: { items: true },
  });
  check(
    'LoggedMeal created through the SAME write path, with vision provenance',
    labelMeal?.source === 'vision' && labelMeal?.visionScanId === labelScan.scanId,
  );
  check(
    "the label's OWN macros were logged verbatim — LogsService trusted them via the existing one-off path",
    labelMeal?.totalCalories === 240 && Number(labelMeal?.totalProteinG) === 5,
  );
  check(
    'the item is a one-off snapshot, not a catalog link',
    labelMeal?.items[0].foodItemId === null && labelMeal?.items[0].nameSnapshot === 'Honey Nut Cereal',
  );
  check(
    'confirmScan needed ZERO changes for a third modality',
    (await prisma.visionScan.findUnique({ where: { id: labelScan.scanId } }))?.status === 'LOGGED',
  );
  ...
  check(
    'new user -> portion is the raw provider estimate (nothing invented)',
    scan1Pollo.portion.grams === 180 && scan1Pollo.portion.method === 'PROVIDER_ESTIMATE',
  );
  check(
    'new user -> explanation has exactly one signal (no phantom priors)',
    scan1Pollo.portionExplanation?.length === 1 && scan1Pollo.portionExplanation?.[0].source === 'VISION',
  );
  ...
  check(
    'the correction is captured as EDITED_PORTION with method attribution (V3.3 column)',
    learnerFeedback?.action === 'EDITED_PORTION' && learnerFeedback?.proposedMethod === 'PROVIDER_ESTIMATE',
  );
  const learnerRatios = await priorReader.visionBiasRatios(learner.id);
  check(
    'the correction corpus is finally READ: ratio 250/180 surfaces for future scans',
    learnerRatios.length === 1 && Math.abs(learnerRatios[0] - 250 / 180) < 1e-9,
  );

  // Two more real meals of the same food at 250g -> 3 validated observations.
  for (let i = 0; i < 2; i++) {
    await logsSvc.logMeal(learner.id, {
      mealType: nowMealType,
      items: [{ foodItemId: pollo.id, quantity: 250, unit: 'g' }],
    } as any);
  }
  ...
  check(
    '3-log prior -> BLENDED estimate, no longer the raw model number',
    scan2Pollo.portion.method === 'BLENDED',
    scan2Pollo.portion.method,
  );
  check(
    'estimate moved toward what the user actually eats',
    Math.abs(scan2Pollo.portion.grams - 250) < Math.abs(180 - 250),
    `${scan2Pollo.portion.grams}g`,
  );
  check(
    'deterministic blend: exactly the arithmetic the docs promise',
    scan2Pollo.portion.grams === 209,
    `${scan2Pollo.portion.grams}g`,
  );
  check(
    'explanation now shows BOTH signals with their weights',
    scan2Pollo.portionExplanation?.length === 2 &&
      !!scan2Pollo.portionExplanation?.find((s: any) => s.source === 'USER_HISTORY'),
  );
  ...
  check(
    '8-log tight prior -> USER_PRIOR: the user outweighs the model',
    scan3Pollo.portion.method === 'USER_PRIOR',
    scan3Pollo.portion.method,
  );
  check(
    'progressively better: scan3 closer to 250g than scan2, scan2 closer than scan1',
    Math.abs(scan3Pollo.portion.grams - 250) < Math.abs(scan2Pollo.portion.grams - 250) &&
      Math.abs(scan2Pollo.portion.grams - 250) < Math.abs(180 - 250),
    `180 -> ${scan2Pollo.portion.grams} -> ${scan3Pollo.portion.grams}`,
  );
  check(
    'portion confidence rose with evidence (0.7 provider -> 0.85 platform-known)',
    scan3Pollo.portion.confidence === 0.85,
  );
  check(
    'Claude became less important WITHOUT any provider change (same fixture, same detections)',
    scan1Pollo.portion.grams === 180 && scan3Pollo.portion.grams !== 180,
  );
  ...
  check(
    'determinism: same history -> identical portion decision',
    JSON.stringify(scan4.candidates[0].portion) === JSON.stringify(scan3Pollo.portion),
  );
  const persistedScan3 = await prisma.visionScan.findUnique({ where: { id: scan3.scanId } });
  check(
    'the persisted proposal carries the deterministic explanation — an audit trail, never a Claude assumption',
    Array.isArray((persistedScan3?.proposal as any)?.candidates?.[0]?.portionExplanation),
  );
  ...
  const day1 = await prisma.mealPlanDay.create({
    data: {
      mealPlanId: plan.id,
      dayNumber: 1,
      targetCalories: 2000,
  ...
  const day2 = await prisma.mealPlanDay.create({
    data: {
      mealPlanId: plan.id,
      dayNumber: 2,
      targetCalories: 2000,
  ...
  const pm1 = await prisma.plannedMeal.create({
    data: {
      mealPlanDayId: day1.id,
      mealType: nowMealType as any,
      name: 'Comida plan A',
      totalCalories: 500,
      totalProteinG: 40,
      totalCarbsG: 30,
  ...
  const pm2 = await prisma.plannedMeal.create({
    data: {
  ...
  await prisma.plannedMealItem.create({
    data: {
      plannedMealId: pm1.id,
      foodItemId: pollo.id,
      amountG: 160,
      calories: 264,
      proteinG: 49.6,
  ...
  check(
    'a scan for a planned user blends the plan expectation (explanation shows it)',
    !!planSignal && planSignal!.grams === 180,
  );
  check(
    'vision agreeing with the plan -> the blend lands on the plan amount',
    planScan.candidates[0].portion.grams === 180,
    `${planScan.candidates[0].portion.grams}g`,
  );

  console.log('\n── V3.3: PRIOR READER RESILIENCE (an enhancement may never fail a scan) ──');
  check(
    'unknown user -> no observations, no crash',
    (await priorReader.userFoodObservations('00000000-0000-0000-0000-000000000000', pollo.id)).length === 0,
  );
  check(
    'unknown user -> no bias, no crash',
    (await priorReader.visionBiasRatios('00000000-0000-0000-0000-000000000000')).length === 0,
  );
  check(
    'no active plan -> null expectation, no crash',
    (await priorReader.plannerExpectedGrams(learner.id, pollo.id, nowMealType)) === null,
  );
  ...
  check(
    'proposal carries restaurant context above the threshold',
    restScan.restaurant?.restaurantName === 'La Esquina Criolla' &&
      restScan.restaurant?.confidence === 0.85 &&
      restScan.restaurant?.category === 'latam casera',
  );
  check(
    'menu candidates arrive from the configured source, sanitized',
    (restScan.restaurant?.menuCandidates.length ?? 0) === 4 && restScan.restaurant!.menuCandidates[0].calories === 620,
  );
  check(
    'detections still matched through the SAME FoodService pipeline (pollo from catalog)',
    restScan.candidates[0].foodItemId === pollo.id,
  );
  check(
    'portion engine untouched: portion + explanation exactly as V3.3 leaves them',
    ['PROVIDER_ESTIMATE', 'SERVING_DEFAULT', 'USER_PRIOR', 'BLENDED'].includes(restScan.candidates[0].portion.method) &&
      Array.isArray(restScan.candidates[0].portionExplanation),
  );
  const restPersisted = await prisma.visionScan.findUnique({ where: { id: restScan.scanId } });
  check(
    'restaurant context persists INSIDE the proposal Json — zero migration',
    (restPersisted?.proposal as any)?.restaurant?.restaurantName === 'La Esquina Criolla',
  );
  ...
  const restConfirm = await visionSvc.confirmScan(user.id, {
    ...
    items: [
      {
        foodItemId: restScan.candidates[0].foodItemId,
        quantity: restScan.candidates[0].portion.grams,
        unit: 'g',
        grams: restScan.candidates[0].portion.grams,
        acceptedFromCandidate: 0,
      },
      {
        foodItemId: null,
        customName: 'Sopa de la casa',
        quantity: 1,
        unit: 'serving',
        calories: 310,
  ...
  const restMeal = await prisma.loggedMeal.findFirst({
    where: { visionScanId: restScan.scanId },
    include: { items: true },
  });
  check(
    'LoggedMeal via the SAME write path with vision provenance (catalog item + menu one-off)',
    restMeal?.source === 'vision' && restMeal?.items.length === 2,
  );
  const soupItem = restMeal?.items.find((i: any) => i.nameSnapshot === 'Sopa de la casa');
  check(
    'menu dish logged as one-off with PUBLISHED calories; unpublished macros stay at the platform default, never invented',
    soupItem?.calories === 310 && Number(soupItem?.proteinG) === 0 && soupItem?.foodItemId === null,
  );
  check(
    'scan LOGGED — confirm needed ZERO changes for a restaurant scan',
    (await prisma.visionScan.findUnique({ where: { id: restScan.scanId } }))?.status === 'LOGGED',
  );
  ...
  check(
    'a weak scene signal (0.3) -> NO restaurant context, ordinary photo scan',
    faintScan.status === 'PROPOSED' && faintScan.restaurant === undefined,
  );
  ...
  check(
    'rejecting a restaurant scan works unchanged',
    (await prisma.visionScan.findUnique({ where: { id: rejectRest.scanId } }))?.status === 'REJECTED',
  );

  console.log('\n── V3.4: MENU SOURCE DEGRADATION (fail-soft, provider swap, determinism) ──');
  const throwingMenuRegistry = new RestaurantMenuProviderRegistry(
    new ConfigService({ RESTAURANT_MENU_PROVIDER: 'throws' }),
    [
      {
        id: 'throws',
        async lookup() {
  ...
  const throwingMenuSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  check(
    'a menu source outage costs the candidates, NEVER the scan (fail-soft)',
    failMenuScan.status === 'PROPOSED' &&
      failMenuScan.restaurant?.restaurantName === 'La Esquina Criolla' &&
      failMenuScan.restaurant?.menuCandidates.length === 0,
  );
  const noneMenuSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
    ...
    ocrRegistry,
    priorReader,
    new RestaurantMenuProviderRegistry(new ConfigService({}), [new NullRestaurantMenuProvider()]),
    trustEngine,
    trustAudit,
    shadowRunner,
  );
  ...
  check(
    "the production default ('none') still surfaces context — only the menu candidates are absent",
    noneScan.restaurant?.restaurantName === 'La Esquina Criolla' && noneScan.restaurant?.menuCandidates.length === 0,
  );
  const restScanAgain = await visionSvc.createScan(user.id, 'restaurant-lunch.jpg', 'PHOTO');
  check(
    'determinism: same image ref -> identical restaurant context',
    JSON.stringify(restScanAgain.restaurant) === JSON.stringify(restScan.restaurant),
  );
  ...
  const liveTrust = new TrustEngine(
    evidenceReader,
    new EvaluationEngine(new GroundTruthReader(prisma), new ReplayEngine(prisma, foodSvc)),
    new ConfigService({ AUTO_ACCEPT_ENABLED: 'true' }),
  );
  const liveSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  const calHealth = await new EvaluationEngine(
    new GroundTruthReader(prisma),
    new ReplayEngine(prisma, foodSvc),
  ).calibration('fixture', { days: 90 });
  check(
    "another user's traffic calibrated the PROVIDER (platform-wide gate, independent of any user's trust)",
    calHealth.curve.builtFrom.examples > 0 && calHealth.curve.bins.some((b: any) => b.n >= 5),
  );
  ...
  check(
    '1st scan: a new user is asked to confirm — trust is EARNED, never granted',
    g1.status === 'PROPOSED' && g1.trust?.action !== 'AUTO_ACCEPT' && g1.trust?.trust.level === 'NONE',
  );
  check(
    '…and the decision is already explainable (signals name the context)',
    !!g1.trust?.signals.includes('NEW_USER') && !!g1.trust?.signals.includes('UNKNOWN_FOOD'),
  );
  const g1Audit = await prisma.visionTrustDecision.findFirst({ where: { scanId: g1.scanId } });
  check(
    '…and persisted for audit from the very first scan',
    g1Audit?.action === 'REVIEW_REQUIRED' && g1Audit?.policyVersion === 1,
  );

  // Confirm the same plate five times. Each confirmation is ground truth.
  const confirmAll = async (p: any) =>
    liveSvc.confirmScan(grad.id, {
      scanId: p.scanId,
      mealType: 'LUNCH',
  ...
  check(
    'AFTER 5 confirmations the platform accepts automatically — the goal, met',
    g6.status === 'LOGGED' && g6.mode === 'AUTO_ACCEPT' && g6.trust?.executed === true,
    `${g6.mode}/${g6.status}`,
  );
  check(
    '…with an undo window and a reason the user can read',
    g6.trust?.undoWindowSeconds === 900 && !!g6.trust?.reason.includes('confirmaciones'),
  );
  check(
    '…and it went through the SAME write path (source=vision, scan-linked)',
    (await prisma.loggedMeal.findFirst({ where: { visionScanId: g6.scanId } }))?.source === 'vision',
  );
  check(
    '…and every candidate on the plate had to be graduated',
    g6.trust?.trust.level === 'HIGH' && g6.candidates.length === 3,
  );
  ...
  check(
    'the auto-accept is explainable forever: policy, evidence, calibration, provider',
    g6Audit?.action === 'AUTO_ACCEPT' &&
      g6Audit?.executed === true &&
      g6Audit?.trustLevel === 'HIGH' &&
      g6Audit?.providerId === 'fixture' &&
      (g6Audit?.calibratedConfidence ?? 0) > 0,
  );
  ...
  check(
    'undo DELETES the meal through LogsService (Vision never touches LoggedMeal)',
    (await prisma.loggedMeal.count({ where: { dailyLog: { userId: grad.id } } })) === beforeUndo - 1,
  );
  check(
    'the scan is marked UNDONE with a specific reason',
    (await prisma.visionScan.findUnique({ where: { id: g6.scanId } }))?.status === 'UNDONE',
  );
  ...
  check(
    'undo is never ignored — it enters the same corpus V3.5 learns from',
    undoFeedback.every((f: any) => f.confirmedFoodItemId !== null),
  );
  ...
  check(
    'after an undo the platform IMMEDIATELY stops auto-accepting — no permanent trust',
    g7.status === 'PROPOSED' && g7.trust?.executed === false && g7.trust?.trust.level === 'NONE',
  );
  check(
    "…and it says exactly why, in the user's language",
    !!g7.trust?.trust.signals.includes('RECENT_UNDO') && !!g7.trust?.reason.includes('deshiciste'),
  );
  ...
  const shadowTrust = new TrustEngine(
    evidenceReader,
    new EvaluationEngine(new GroundTruthReader(prisma), new ReplayEngine(prisma, foodSvc)),
    new ConfigService({}),
  );
  const shadowSvc = new VisionScanService(
    prisma,
    foodSvc,
    logsSvc,
    registry,
  ...
  check(
    'with AUTO_ACCEPT_ENABLED unset the platform still DECIDES auto-accept…',
    shadowScan.trust?.action === 'AUTO_ACCEPT' && shadowScan.trust?.trust.level === 'HIGH',
  );
  check(
    '…but never acts: status stays PROPOSED, the user still confirms',
    shadowScan.status === 'PROPOSED' && shadowScan.trust?.executed === false && shadowScan.mode !== 'AUTO_ACCEPT',
  );
  check(
    '…and the shadow decision is persisted, so the rollout can be measured before it is enabled',
    (await prisma.visionTrustDecision.findFirst({ where: { scanId: shadowScan.scanId } }))?.executed === false,
  );
  ...
  const nonVisionScanRows = await prisma.loggedMeal.count({
    where: { dailyLog: { userId: user.id }, visionScanId: null, source: 'vision' },
  });
  check(
    'every vision-sourced meal has a scan link (no direct vision write bypassed LogsService)',
    nonVisionScanRows === 0 && visionMeals >= 1,
    `vision meals=${visionMeals}`,
  );
