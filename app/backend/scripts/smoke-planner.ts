...
function mkCtx(over: any = {}): CoachingContext {
  const base: CoachingContext = {
    meta: {
      contract: 'vitals-fit.coaching-context',
      version: 1,
      depth: 'full',
...
async function main() {
  ...
  check(
    '1 completed week -> WAIT_FOR_MORE_DATA / INSUFFICIENT_DATA',
    insufficient.headline.code === 'WAIT_FOR_MORE_DATA' && insufficient.posture === 'INSUFFICIENT_DATA',
    `${insufficient.posture}`,
  );
  check(
    'WAIT decision names the data gap',
    insufficient.headline.evidence.some((e) => e.code === 'INSUFFICIENT_HISTORY'),
  );

  console.log('\n── CALORIES: LOSE ──');
  const plateau = decidePlan(mkCtx()); // 3 stalled weeks, plateauNow, high adherence
  check(
    'sustained plateau + high adherence -> REDUCE_CALORIES',
    plateau.headline.code === 'REDUCE_CALORIES',
    plateau.headline.code,
  );
  check('REDUCE is HIGH confidence at 3 weeks', plateau.headline.confidence === 'HIGH', plateau.headline.confidence);
  check(
    'REDUCE carries -200 kcal adjustment',
    plateau.headline.adjustment?.calorieDelta === -200 && plateau.headline.adjustment?.newCalorieTarget === 1800,
  );
  check('REDUCE review window = 14 days', plateau.headline.reviewWindowDays === 14);
  check('posture EVOLVING', plateau.posture === 'EVOLVING');
  check(
    'evidence cites sustained plateau + high adherence',
    plateau.headline.evidence.some((e) => e.code === 'PLATEAU_SUSTAINED') &&
      plateau.headline.evidence.some((e) => e.code === 'HIGH_ADHERENCE'),
  );

  const lowAdh = decidePlan(
    mkCtx({
      currentState: { plateauStatus: 'NONE', adherenceScore: 50 },
      history: {
        weeks: [
          mkWeek({ adherenceScore: 50, plateauStatus: 'NONE' }),
          mkWeek({ adherenceScore: 48, plateauStatus: 'NONE' }),
          mkWeek({ adherenceScore: 52, plateauStatus: 'NONE' }),
        ],
      },
    }),
  );
  check(
    'sustained stall + LOW adherence -> KEEP_PLAN (adherence first)',
    lowAdh.headline.code === 'KEEP_PLAN' && lowAdh.posture === 'ADHERENCE_FIRST',
    `${lowAdh.headline.code}/${lowAdh.posture}`,
  );
  check(
    'adherence-first cites LOW_ADHERENCE, no calorie change',
    lowAdh.headline.adjustment === null && lowAdh.headline.evidence.some((e) => e.code === 'LOW_ADHERENCE'),
  );

  const fastLoss = decidePlan(
    mkCtx({ currentState: { weight: { currentKg: 78, trendKgPerWeek: -1.2, dataPoints: 4 } } }),
  );
  check(
    'losing too fast -> INCREASE_CALORIES',
    fastLoss.headline.code === 'INCREASE_CALORIES' && fastLoss.headline.adjustment?.calorieDelta === 100,
    fastLoss.headline.code,
  );

  const onTrack = decidePlan(
    mkCtx({
      currentState: { plateauStatus: 'NONE', weight: { currentKg: 79, trendKgPerWeek: -0.5, dataPoints: 4 } },
      history: {
        weeks: [
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE' }),
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE' }),
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE' }),
        ],
      },
  ...
  check(
    'on track -> KEEP_PLAN / STABLE',
    onTrack.headline.code === 'KEEP_PLAN' && onTrack.posture === 'STABLE',
    `${onTrack.headline.code}/${onTrack.posture}`,
  );

  const fewWeightPts = decidePlan(
    mkCtx({ currentState: { weight: { currentKg: 80, trendKgPerWeek: 0, dataPoints: 1 } } }),
  );
  check(
    'enough weeks but too few weight points -> calorie WAIT / INSUFFICIENT_DATA',
    fewWeightPts.headline.code === 'WAIT_FOR_MORE_DATA' && fewWeightPts.posture === 'INSUFFICIENT_DATA',
  );

  console.log('\n── CALORIES: GAIN ──');
  const gainStall = decidePlan(
    mkCtx({
      user: { goal: 'gain' },
      currentState: { plateauStatus: 'NONE', weight: { currentKg: 75, trendKgPerWeek: 0.0, dataPoints: 4 } },
      history: {
        weeks: [
          mkWeek({ trendStatus: 'stalled', plateauStatus: 'NONE' }),
          mkWeek({ trendStatus: 'stalled', plateauStatus: 'NONE' }),
          mkWeek({ trendStatus: 'stalled', plateauStatus: 'NONE' }),
        ],
      },
  ...
  check(
    'gain stalled + high adherence -> INCREASE_CALORIES',
    gainStall.headline.code === 'INCREASE_CALORIES' && gainStall.headline.adjustment?.calorieDelta === 100,
    gainStall.headline.code,
  );
  ...
  check(
    'a calorie cut is complemented by INCREASE_PROTEIN',
    protDec?.code === 'INCREASE_PROTEIN' && protDec?.adjustment?.proteinDelta === 15,
    protDec?.code,
  );
  check('headline stays the calorie cut, not the protein bump', cutProtein.headline.code === 'REDUCE_CALORIES');

  const reduceProtein = decidePlan(
    mkCtx({
      currentState: { plateauStatus: 'NONE', weight: { currentKg: 79, trendKgPerWeek: -0.4, dataPoints: 4 } },
      history: {
        weeks: [
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE', avgProtein: 70 }),
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE', avgProtein: 68 }),
          mkWeek({ trendStatus: 'on_track', plateauStatus: 'NONE', avgProtein: 72 }),
        ],
      },
    }),
  );
  ...
  check(
    'adherent but protein chronically unreachable -> REDUCE_PROTEIN',
    rp?.code === 'REDUCE_PROTEIN' && rp?.adjustment?.newProteinTarget === 135,
    `${rp?.code}`,
  );

  console.log('\n── INTERVENTION ──');
  const replace = decidePlan(
    mkCtx({
      currentState: { plateauStatus: 'NONE', weight: { currentKg: 79, trendKgPerWeek: -0.4, dataPoints: 4 } },
      history: {
        weeks: [
      ...
      review: {
        ...
        improvedMetrics: [],
        worsenedMetrics: [],
        biggestOpportunity: 'PROTEIN_CHRONIC_LOW',
        biggestImprovement: null,
        commitmentOutcomes: [],
        followUp: {
          resolved: [],
          persisting: [{ issue: 'PROTEIN_CHRONIC_LOW', weeksActive: 2, intervention: 'INTERVENED' }],
          emerged: [],
        },
        nextPriority: { reason: 'PROTEIN_CHRONIC_LOW', basis: 'PERSISTENT_INTERVENED' },
      },
    }),
  );
  check(
    'committed intervention still failing -> REPLACE_INTERVENTION',
    replace.headline.code === 'REPLACE_INTERVENTION' && replace.posture === 'EVOLVING',
    replace.headline.code,
  );
  check(
    'REPLACE cites the failed intervention',
    replace.headline.evidence.some((e) => e.code === 'INTERVENTION_FAILED'),
  );

  console.log('\n── DETERMINISM ──');
  check(
    'identical context -> identical plan',
    JSON.stringify(decidePlan(mkCtx())) === JSON.stringify(decidePlan(mkCtx())),
  );
  ...
  const logDay = (d: Date) =>
    prisma.dailyLog.create({
      data: {
        ...
        loggedMeals: {
          create: [
            { mealType: 'BREAKFAST' as const, totalCalories: 700, totalProteinG: 60, totalCarbsG: 60, totalFatG: 18 },
            { mealType: 'LUNCH' as const, totalCalories: 1280, totalProteinG: 90, totalCarbsG: 140, totalFatG: 42 },
          ],
        },
      },
    });
  ...
  for (const [d, kg] of [
    [addDaysUTC(wB, 0), 80.0],
    [addDaysUTC(wB, 4), 80.0],
    [addDaysUTC(wC, 0), 80.0],
    [addDaysUTC(wC, 4), 80.0],
  ] as const) {
  ...
  check(
    'weeksAnalyzed reflects the ledger (2 completed weeks)',
    plan.meta.weeksAnalyzed === 2,
    `${plan.meta.weeksAnalyzed}`,
  );
  check(
    'posture is a valid value',
    ['STABLE', 'EVOLVING', 'ADHERENCE_FIRST', 'INSUFFICIENT_DATA'].includes(plan.posture),
    plan.posture,
  );
  check(
    'plan is deterministic on rebuild',
    JSON.stringify(strip(await planner.getPlan(user.id))) === JSON.stringify(strip(plan)),
  );
